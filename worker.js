...
async function writeSaveRecord(env, name, data) {
...
async function readSaveRecord(env, name) {
  ...
  if (looksLikeAesEnvelope(raw)) {
    return await aesGcmDecryptFromJson(env, raw);
  }
...
export class GlobalState {
  ...
  async settleTrade(offer) {
    ...
    await writeSaveRecord(this.env, offer.fromName, fromData);
    await writeSaveRecord(this.env, offer.toName, toData);
...
export class GuildState {
  ...
  async fetch(request) {
    ...
    if (path.endsWith('/season/claim') && request.method === 'POST') {
      ...
      await writeSaveRecord(this.env, name, data);
...
async function runAdminCommand(env, cmd, args) {
  ...
  switch (cmd) {
    ...
    case 'setLevel': {
      ...
      await writeSaveRecord(env, name, data);
