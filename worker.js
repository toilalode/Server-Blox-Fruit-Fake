...
/* ============================================================
   ...
   Luồng (v3 — NHÚNG SAVE DATA + DÙNG 1 LẦN:
     ticket = { accountId, ts, nonce, data }
     signed = HMAC-SHA256( accountId + ':' + ts + ':' + nonce + ':' + JSON.stringify(data) )  ký bằng ADMIN_KEY
     EXPORT CODE = base64url(JSON.stringify({ accountId, ts, nonce, data, signed }))
   Verify: tính lại HMAC trên đúng {accountId, ts, nonce, data} lấy từ code -> so với signed đi kèm.
   Vì HMAC dùng secret CHỈ SERVER BIẾT (ADMIN_KEY) nên: (1) client không tự chế mã giả cho tên người
   khác, (2) ai sửa tay dù 1 ký tự trong `data` (gold/level...) sau khi export cũng khiến signed không
   khớp -> bị từ chối ngay, không cần server đọc lại KV để so sánh.
   Import CHỈ CẦN gọi server để verify chữ ký (không cần server tồn tại KV save cho tên đó nữa) — do đó
   vẫn cần mạng (để verify), nhưng KHÔNG còn phụ thuộc server còn lưu save hay không.
   ...
   ts dùng để hiển thị "mã tạo lúc nào"; hiện KHÔNG áp hạn theo thời gian (chỉ hạn theo "đã dùng chưa").
============================================================ */
...
// Chuỗi dùng làm input ký/verify HMAC — GHÉP CỐ ĐỊNH accountId + ts + nonce + JSON của data, để bất kỳ
// thay đổi nào ở data (dù chỉ 1 ký tự — sửa gold/level bằng tay) hoặc ở nonce đều làm input đổi -> chữ
// ký không khớp nữa.
function exportTicketMessage(accountId, ts, nonce, data) {
  return accountId + ':' + ts + ':' + nonce + ':' + JSON.stringify(data === undefined ? null : data);
}
...
async function signExportTicket(env, accountId, ts, nonce, data) {
  ...
  const sig = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(exportTicketMessage(accountId, ts, nonce, data)));
  return bufToBase64Url(sig);
}

// Tạo export code hoàn chỉnh cho 1 accountId, NHÚNG LUÔN save `data` hiện tại + 1 nonce ngẫu nhiên mới
// (dùng để về sau đánh dấu chính XÁC mã này đã được import hay chưa) vào trong mã.
async function createExportCode(env, accountId, data) {
  const ts = Date.now();
  const nonce = genExportNonce();
  const signed = await signExportTicket(env, accountId, ts, nonce, data);
  const ticket = { accountId, ts, nonce, data: data === undefined ? null : data, signed };
...
// mạo/dữ liệu bị sửa tay/mã đã dùng rồi. Trả về { accountId, ts, nonce, data } nếu hợp lệ — KHÔNG cần
// đọc KV save, vì data đã nằm sẵn trong chính mã (đã được ký nên không thể bị sửa mà không bị phát hiện).
...
async function verifyExportCode(env, code) {
  ...
  const { accountId, ts, nonce, data, signed } = ticket || {};
  if (!accountId || typeof accountId !== 'string' || !ts || !nonce || !signed) {
    throw new Error('Mã import thiếu dữ liệu bắt buộc (có thể là mã kiểu cũ trước khi có cơ chế 1 lần — hãy Xuất lại mã mới).');
  }
  const expected = await signExportTicket(env, accountId, ts, nonce, data);
  ...
  // Chữ ký hợp lệ -> kiểm tra xem nonce này đã từng được đánh dấu "đã dùng" chưa.
