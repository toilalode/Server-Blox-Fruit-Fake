...
async function checkRateLimit(env, endpoint, ip, maxRequests, windowSeconds) {
  ...
  const key = 'ratelimit:' + endpoint + ':' + ip + ':' + windowIndex;
  const currentRaw = await env.GAME_DATA.get(key);
  ...
  env.GAME_DATA.put(key, String(current + 1), { expirationTtl: windowSeconds }).catch(() => {});
  return true;
}
...
async function writeMergedPlayerRecord(env, name, { saveData, metaData, nameRecord } = {}) {
  ...
  if (metaData !== undefined) {
    writes.push(env.GAME_DATA.put('meta:' + name, JSON.stringify(metaData)));
  }
  if (nameRecord !== undefined) {
    writes.push(env.GAME_DATA.put('name:' + name, JSON.stringify(nameRecord)));
  }
    ...
    try {
      ...
      if (path === '/account/rename' && request.method === 'POST') {
        ...
        const writes = [
          ...
          env.GAME_DATA.put('meta:' + newName, JSON.stringify({ ip, deviceId: newDeviceId, updatedAt: Date.now() })),
        ];
        ...
        await Promise.all([
          env.GAME_DATA.delete('name:' + name),
          env.GAME_DATA.delete('save:' + name),
          env.GAME_DATA.delete('meta:' + name),
          env.GAME_DATA.delete('lb:' + name),
        ]);
