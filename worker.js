...
// Bộ lọc từ cấm cơ bản cho chat 'moderated' — chỉ chặn được ở SERVER vì client có thể bị sửa để bypass.
// Danh sách rút gọn, có thể mở rộng thêm; so khớp không phân biệt hoa thường và bỏ dấu cách thừa.
const BANNED_WORDS = ['dm','vcl','vl','đm','cc','clm','loz','lol','đù má','đéo','ngu','óc chó'];
...
/* ============================================================
   ...
   - Sự kiện toàn cục (x50, hidden/coming soon)
...
const TEMP_ADMIN_ALLOWED_CMDS = new Set([
  ...
  'setCamera', 'setHp', 'setMana', 'addFruitBag', 'addFruitTemp', 'kick', 'unkick', 'addEvent',
]);
    ...
    try {
      ...
      if (roomMatch) {
        ...
        if (roomMatch[2] === 'ws') {
          ...
          if (banReason) return json({ error: banReason }, 403);
          if (wsName && await env.GAME_DATA.get('kicked:' + wsName)) {
      ...
      //   setHp, setMana, ban, banHwid, unban, kick, unkick, addFruitBag, addFruitTemp, addTempAdmin,
      //   removeTempAdmin, addEvent, broadcast — xem chi tiết args trong runAdminCommand() bên trên.
