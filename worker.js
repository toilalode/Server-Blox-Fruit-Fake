 ...
 *   - Secret  tên "EXPORT_WRAP_KEY"  -> lớp bọc HMAC-SHA256 NGOÀI CÙNG cho mã export/import, ĐỘC LẬP
 *     hoàn toàn với ADMIN_KEY (không dẫn xuất từ nó). CHỈ SERVER BIẾT — KHÔNG xuất hiện trong worker.js
 *     lẫn index.html (chỉ tên biến env được tham chiếu trong code, không phải giá trị thật).
...
/* ============================================================
   ...
   Luồng v4 — MÃ HOÁ AES-GCM (IV ngẫu nhiên) + KÝ HMAC + HẾT HẠN + DÙNG 1 LẦN:
     key    = HKDF-SHA256(ADMIN_KEY) dẫn xuất riêng cho mục đích export (tách biệt khỏi key ký HMAC)
     ...
     signed = HMAC-SHA256( accountId + ':' + ts + ':' + nonce + ':' + iv + ':' + cipher )  ký bằng ADMIN_KEY
   ...
   RỒI mới giải mã AES-GCM. Vì cả 2 lớp (mã hoá + chữ ký) đều dùng secret CHỈ SERVER BIẾT (ADMIN_KEY) nên:
   ...
   Secret (ADMIN_KEY) KHÔNG BAO GIỜ gửi xuống client/HTML — toàn bộ mã hoá/ký/giải mã/verify chỉ chạy trên
   ...
   EXPORT_WRAP_KEY — secret này ĐỘC LẬP với ADMIN_KEY (không dẫn xuất từ nó, không có mặt trong bất kỳ
...
/* ---- LỚP BỌC NGOÀI CÙNG: HMAC-SHA256 với secret RIÊNG "EXPORT_WRAP_KEY" -------------------------------
   Đây là secret THỨ HAI, ĐỘC LẬP HOÀN TOÀN với ADMIN_KEY — không dẫn xuất từ ADMIN_KEY, không lưu trong
   ...
   Mục đích: phòng thủ theo lớp (defense in depth) — nếu ADMIN_KEY (dùng để ký + mã hoá AES-GCM ở trên) vì
...
async function signOuterWrap(env, payload) {
  if (!env.EXPORT_WRAP_KEY) throw new Error('Server chưa cấu hình khoá bọc ngoài (EXPORT_WRAP_KEY).');
  const key = await crypto.subtle.importKey(
    'raw', new TextEncoder().encode(env.EXPORT_WRAP_KEY),
    { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const sig = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(payload));
  return bufToBase64Url(sig);
}
// Bọc 1 mã (chuỗi base64url của ticket AES-GCM+HMAC bên trong) thêm 1 lớp HMAC-SHA256 ngoài cùng.
async function wrapOuterLayer(env, innerCode) {
  const wrap = await signOuterWrap(env, innerCode);
  return textToBase64Url(JSON.stringify({ p: innerCode, w: wrap }));
}
// Bóc lớp bọc ngoài — verify HMAC (secret EXPORT_WRAP_KEY) TRƯỚC KHI đụng tới bất kỳ phần nào của ticket
// bên trong. Trả về innerCode (chuỗi base64url gốc) nếu hợp lệ; ném lỗi rõ ràng nếu sai/thiếu lớp bọc.
async function unwrapOuterLayer(env, code) {
  ...
  const expected = await signOuterWrap(env, p);
...
// Ký HMAC-SHA256 bằng secret ADMIN_KEY (đã có sẵn trên server, không cần secret mới).
...
async function signExportTicket(env, accountId, ts, nonce, iv, cipher) {
  if (!env.ADMIN_KEY) throw new Error('Server chưa cấu hình khoá ký (ADMIN_KEY).');
  const key = await crypto.subtle.importKey(
...
// Dẫn xuất khoá AES-256-GCM RIÊNG cho mục đích mã hoá export (tách biệt hoàn toàn khỏi khoá dùng để ký
// HMAC ở trên, dù cùng bắt nguồn từ 1 secret ADMIN_KEY) bằng HKDF-SHA256 với info cố định 'export-aes-gcm'
// — cách chuẩn để tạo nhiều khoá độc lập từ 1 secret gốc mà không cần lưu thêm secret nào khác trên server.
async function deriveExportAesKey(env) {
  if (!env.ADMIN_KEY) throw new Error('Server chưa cấu hình khoá mã hoá (ADMIN_KEY).');
  const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(env.ADMIN_KEY), 'HKDF', false, ['deriveKey']);
...
async function deriveSaveAesKey(env, secretOverride) {
  const secret = secretOverride !== undefined ? secretOverride : env.EXPORT_WRAP_KEY;
  if (!secret) throw new Error('Server chưa cấu hình khoá mã hoá dữ liệu (EXPORT_WRAP_KEY).');
...
// Mã hoá 1 object bất kỳ (save data, hoặc chuỗi wrap ngoài của export code) bằng AES-GCM, IV ngẫu nhiên
// 12 byte mới mỗi lần gọi. Trả về chuỗi JSON string sẵn sàng để `put` thẳng vào KV hoặc nhúng vào ticket.
// LUÔN mã hoá bằng EXPORT_WRAP_KEY HIỆN TẠI (không bao giờ dùng khoá cũ để ghi mới) — key rotation chỉ ảnh
// hưởng chiều GIẢI MÃ (xem aesGcmDecryptFromJson bên dưới).
async function aesGcmEncryptToJson(env, data) {
  const key = await deriveSaveAesKey(env);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const plaintext = new TextEncoder().encode(JSON.stringify(data === undefined ? null : data));
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plaintext);
...
/* ---- KEY ROTATION cho EXPORT_WRAP_KEY -----------------------------------------------------------------
   Nếu nghi ngờ EXPORT_WRAP_KEY bị lộ (hoặc định kỳ đổi khoá theo chính sách bảo mật), quy trình xoay khoá
   AN TOÀN không làm mất dữ liệu đã mã hoá bằng khoá cũ:
     1. wrangler secret put EXPORT_WRAP_KEY_PREV   -> đặt giá trị = khoá CŨ hiện tại (trước khi đổi)
     2. wrangler secret put EXPORT_WRAP_KEY        -> đặt giá trị = khoá MỚI
     3. Deploy lại Worker. Từ giờ: GHI MỚI luôn dùng khoá MỚI; GIẢI MÃ tự thử khoá MỚI trước, nếu auth tag
        không khớp (dữ liệu cũ) thì tự động thử lại bằng EXPORT_WRAP_KEY_PREV trước khi báo lỗi thật sự.
     4. (Tuỳ chọn, sau khi chắc chắn phần lớn dữ liệu đã được ghi lại ít nhất 1 lần bằng khoá mới — mỗi lần
        /save đều ghi lại bằng khoá mới — có thể chạy /admin/reencrypt-all để chủ động đọc+ghi lại TOÀN BỘ
        KV bằng khoá mới, rồi xoá EXPORT_WRAP_KEY_PREV để không còn chấp nhận khoá cũ nữa.)
   KHÔNG xoá EXPORT_WRAP_KEY_PREV ngay sau bước 2 — làm vậy sẽ khiến MỌI dữ liệu save/export CŨ (mã hoá
   bằng khoá cũ) không giải mã được nữa, coi như mất dữ liệu vĩnh viễn. --------------------------------- */
async function aesGcmDecryptFromJson(env, jsonStr) {
  ...
  try {
    ...
    // Khoá hiện tại không giải mã được (auth tag sai) — thử khoá CŨ nếu server có cấu hình rotation.
    if (!env.EXPORT_WRAP_KEY_PREV) throw errCurrent;
    const prevKey = await deriveSaveAesKey(env, env.EXPORT_WRAP_KEY_PREV);
    const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ivBytes }, prevKey, cipherBytes);
    return JSON.parse(new TextDecoder().decode(plainBuf));
  }
}
...
// SAU ĐÓ bọc thêm 1 lớp HMAC-SHA256 ngoài cùng bằng secret ĐỘC LẬP EXPORT_WRAP_KEY (xem giải thích ở trên).
async function createExportCode(env, accountId, data) {
  ...
  const hmacWrapped = await wrapOuterLayer(env, innerCode);
  // LỚP THỨ TƯ (NGOÀI CÙNG THẬT SỰ): mã hoá AES-GCM chuỗi đã ký HMAC ở trên, dùng khoá dẫn xuất từ
  // EXPORT_WRAP_KEY qua module aesGcmEncryptToJson/deriveSaveAesKey (info='save-data-aes-gcm-v1', dùng
  // CHUNG với lớp mã hoá save data trong KV). Kẻ tấn công không có EXPORT_WRAP_KEY thì không đọc được
  // NỘI DUNG mã export (kể cả chữ ký HMAC/accountId/nonce) chứ không chỉ bị chặn xác thực — trước đây lớp
  // HMAC ngoài cùng chỉ XÁC THỰC, không MÃ HOÁ, nên nội dung base64url vẫn đọc/giải mã ngược được nếu có
  // ADMIN_KEY dù không có EXPORT_WRAP_KEY. Giờ cả 2 secret đều bắt buộc phải đúng.
  const aesEnvelopeJson = await aesGcmEncryptToJson(env, hmacWrapped);
...
async function verifyExportCode(env, code) {
  // Bóc lớp NGOÀI CÙNG THẬT SỰ trước tiên: giải mã AES-GCM (khoá dẫn xuất từ EXPORT_WRAP_KEY) — sai khoá
  // hoặc mã bị sửa dù 1 byte thì auth tag chặn ngay tại đây, KHÔNG cần đụng tới HMAC/AES-GCM bên trong.
  let hmacWrapped;
  try {
    const aesEnvelopeJson = base64UrlToText(String(code || '').trim());
    hmacWrapped = await aesGcmDecryptFromJson(env, aesEnvelopeJson);
  } catch (err) {
    throw new Error('Mã import không hợp lệ (sai lớp bọc ngoài cùng — có thể mã bị sửa hoặc không phải do server này cấp).');
  }
  // Sau đó bóc lớp HMAC-SHA256 (cũng secret EXPORT_WRAP_KEY) — giữ nguyên như trước để không phá luồng
  // xác thực hiện có bên trong.
  const innerCode = await unwrapOuterLayer(env, hmacWrapped);
  let ticket;
  try {
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Xác minh mã export (chữ ký ADMIN_KEY, bao trùm cả accountId+ts+data nên không sửa tay được) ->
      ...
      if (path === '/save' && request.method === 'POST') {
        ...
        await writeMergedPlayerRecord(env, name, {
          ...
          nameRecord: nameRecordToWrite, // undefined nếu không cần đổi (giữ nguyên bản KV hiện có)
        });
