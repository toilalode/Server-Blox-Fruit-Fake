...
async function createExportCode(env, accountId, data) {
  ...
  return wrapOuterLayer(env, innerCode);
}
...
async function verifyExportCode(env, code) {
  // Bóc lớp bọc ngoài (HMAC-SHA256, secret EXPORT_WRAP_KEY) TRƯỚC TIÊN — sai lớp này bị từ chối ngay,
  // không cần đụng tới AES-GCM/HMAC bên trong.
  const innerCode = await unwrapOuterLayer(env, code);
  let ticket;
  try {
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/account/rename' && request.method === 'POST') {
        ...
        const writes = [env.GAME_DATA.put('name:' + newName, oldRecordRaw)];
        if (oldSaveRaw) writes.push(env.GAME_DATA.put('save:' + newName, oldSaveRaw));
        if (oldMetaRaw) writes.push(env.GAME_DATA.put('meta:' + newName, oldMetaRaw));
        ...
        await recordPlayerMeta(env, newName, request, body.deviceId);
      ...
      if (path === '/account/export' && request.method === 'POST') {
        ...
        if (dataToEmbed === undefined || dataToEmbed === null) {
          const raw = await env.GAME_DATA.get('save:' + name);
          dataToEmbed = raw ? JSON.parse(raw) : null;
        }
        let code;
      ...
      if (path === '/account/import' && request.method === 'POST') {
        ...
        if (banReason) return json({ error: banReason }, 403);
        const raw = await env.GAME_DATA.get('name:' + accountId);
        if (!raw) return json({ error: 'Tài khoản "' + accountId + '" không còn tồn tại trên server (có thể đã bị xoá).' }, 404);
        let record;
        try { record = JSON.parse(raw); } catch (err) { record = {}; }
        // Idempotent giống /account/claim-token: nếu record cũ chưa có token thì cấp mới, còn có rồi thì trả luôn.
        if (!record.token) {
          record.token = genToken();
          await env.GAME_DATA.put('name:' + accountId, JSON.stringify(record));
        }
        await recordPlayerMeta(env, accountId, request, body.deviceId);
        const saveRaw = await env.GAME_DATA.get('save:' + accountId);
        const serverData = saveRaw ? JSON.parse(saveRaw) : null;
