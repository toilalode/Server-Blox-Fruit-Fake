 ...
 *   - Secret  tên "ADMIN_KEY"  -> mật khẩu admin thật, CHỈ SERVER BIẾT. CHỈ dùng để xác thực các
 *     endpoint /admin/* (verify, command, event/set, log) — KHÔNG còn liên quan gì tới export/import.
 *     Set bằng: wrangler secret put ADMIN_KEY   (hoặc Dashboard > Settings > Variables > Secret)
 *   - Secret  tên "EXPORT_SIGN_KEY"  -> khoá ký HMAC-SHA256 cho ticket export/import, ĐỘC LẬP hoàn toàn
 *     với ADMIN_KEY và EXPORT_AES_KEY/EXPORT_WRAP_KEY (không dẫn xuất từ secret nào khác).
 *     Set bằng: wrangler secret put EXPORT_SIGN_KEY
 *   - Secret  tên "EXPORT_AES_KEY"  -> khoá gốc để dẫn xuất khoá mã hoá AES-GCM cho dữ liệu NHÚNG trong
 *     mã export/import, ĐỘC LẬP hoàn toàn với ADMIN_KEY và EXPORT_SIGN_KEY/EXPORT_WRAP_KEY.
 *     Set bằng: wrangler secret put EXPORT_AES_KEY
 *   - Secret  tên "EXPORT_MID_AES_KEY" (MỚI) -> lớp AES-GCM Ở GIỮA cho export/import, nằm ngay sau
 *     innerCode (base64url của ticket AES+HMAC-sign) và TRƯỚC lớp HMAC-wrap + AES-GCM-wrap ngoài cùng.
 *     ĐỘC LẬP HOÀN TOÀN với mọi secret khác (không dẫn xuất từ nhau). Không bắt buộc — nếu server chưa
 *     set, mã export CŨ (phát hành trước khi có lớp này) vẫn đọc được bình thường (tự phát hiện thiếu lớp
 *     giữa và bỏ qua), nhưng KHÔNG tạo được export MỚI cho tới khi secret này được cấu hình.
 *     Set bằng: wrangler secret put EXPORT_MID_AES_KEY
 *   - Secret  tên "EXPORT_WRAP_KEY_CURRENT"  -> lớp bọc HMAC-SHA256 + AES-GCM NGOÀI CÙNG cho mã export/
 *     import MỚI, ĐỘC LẬP hoàn toàn với ADMIN_KEY/EXPORT_SIGN_KEY/EXPORT_AES_KEY (không dẫn xuất từ nhau).
 *     Set bằng: wrangler secret put EXPORT_WRAP_KEY_CURRENT
 *   - Secret  tên "EXPORT_WRAP_KEY_PREV_1" / "EXPORT_WRAP_KEY_PREV_2" (TUỲ CHỌN) -> khoá bọc ngoài THẾ HỆ
 *     TRƯỚC, chỉ dùng để ĐỌC mã export cũ trong migration window sau khi rotate CURRENT — KHÔNG BAO GIỜ
 *     dùng để tạo export mới. Xem KEY ROTATION ngay trên EXPORT_WRAP_KEY_RING trong code.
 *   - Secret  tên "EXPORT_WRAP_KEY" (LEGACY, tuỳ chọn) -> secret bọc ngoài THẾ HỆ CŨ (trước khi có key
 *     ring versioned), giữ lại tạm thời CHỈ để đọc mã export phát hành trước migration; có thể xoá sau
 *     khi hết migration window (>24h từ lúc ngừng phát hành theo secret cũ, vì EXPORT_CODE_MAX_AGE_MS=24h).
 *     Set bằng: wrangler secret put EXPORT_WRAP_KEY   (hoặc Dashboard > Settings > Variables > Secret)
 *   - Secret  tên "SAVE_DATA_KEY" (MỚI) -> mã hoá/giải mã TOÀN BỘ save data ('save:'+tên) trong KV, ĐỘC
 *     LẬP HOÀN TOÀN với mọi secret khác ở trên (không dẫn xuất từ nhau). Tách riêng khỏi EXPORT_WRAP_KEY
 *     để lộ 1 trong 2 không ảnh hưởng cái còn lại (trước đây 2 mục đích dùng chung 1 secret).
 *     Set bằng: wrangler secret put SAVE_DATA_KEY
 *     Rotation (nếu nghi lộ): set SAVE_DATA_KEY_PREV = khoá cũ, rồi set SAVE_DATA_KEY = khoá mới, deploy lại
 *     — xem chi tiết ngay phía trên hàm deriveSaveAesKey/aesGcmDecryptFromJson.
 *   - Secret  tên "ADMIN_PROTECT_SECRET" (MỚI) -> lớp 2 cho xác thực admin, ĐỘC LẬP với ADMIN_KEY. Nếu
 *     server chưa set thì lớp 2 tự tắt (không khoá cứng admin hiện tại) — NÊN set trên production.
 *     Set bằng: wrangler secret put ADMIN_PROTECT_SECRET
 *
 *   => Export/import dùng 3 secret tách biệt khỏi ADMIN_KEY: EXPORT_SIGN_KEY, EXPORT_AES_KEY, EXPORT_WRAP_KEY.
 *      Save data dùng secret RIÊNG: SAVE_DATA_KEY. Admin dùng 2 lớp: ADMIN_KEY + ADMIN_PROTECT_SECRET.
 *      Lộ 1 secret không ảnh hưởng các secret còn lại.
 *
 * Endpoint cũ: /check-name /register-name /save /load /leaderboard/top
 *   /account/rename (POST {name,newName,token,deviceId} -> {name,token}) — đổi TÊN THẬT tài khoản trên
 *   server, giữ nguyên token/save/tiến trình ("Đổi tên không mất account"); chuyển toàn bộ key KV
 *   name:/save:/meta: sang tên mới rồi xoá key cũ.
 *   /account/export (POST {name,token,deviceId,data} -> {code})  và
 *   /account/import (POST {code} -> {name,token,data,usedEmbeddedData}) — chuyển tài khoản sang máy/
 *   trình duyệt khác bằng 1 mã ký HMAC(ADMIN_KEY) có NHÚNG LUÔN save data vào mã (không cần token cũ
 *   lưu localStorage, và Import không bắt buộc phải phụ thuộc server còn giữ 'save:'+tên hay không —
 *   server ưu tiên bản mới nhất trên KV nếu còn, dùng bản nhúng trong mã nếu không còn). MỖI MÃ CHỈ
 *   DÙNG ĐƯỢC 1 LẦN (nonce ngẫu nhiên trong ticket, đánh dấu 'used-export:'+nonce trong KV sau khi
 *   import thành công) — dùng lại mã cũ sẽ bị từ chối dù chữ ký vẫn đúng. Xem
 *   verifyExportCode/createExportCode/signExportTicket/markExportCodeUsed.
...
// MỤC 23 (hardening): trước đây Allow-Origin là '*' cho MỌI response (kể cả /admin/*). Giữ CORS_HEADERS
// làm giá trị MẶC ĐỊNH/dự phòng (dùng nội bộ, dùng cho preflight khi chưa biết origin) — nhưng response
// THẬT SỰ trả về trình duyệt được hậu xử lý bởi applyCors() bên dưới, so khớp Origin với ALLOWED_ORIGINS
// rồi mới quyết định giá trị Allow-Origin cuối cùng. Request KHÔNG có header Origin (app WebView/Cordova,
// gọi server-to-server, curl...) không bị ảnh hưởng bởi CORS (trình duyệt mới là bên áp CORS), nên không
// cần lo che giấu domain cho các trường hợp đó.
const CORS_HEADERS = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'GET,POST,OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Admin-Session',
};
// Danh sách domain CHÍNH THỨC được phép gọi API bằng fetch() từ trình duyệt (CORS). Cập nhật danh sách
// này nếu game được host thêm ở domain khác. localhost/127.0.0.1 giữ lại để tiện test cục bộ.
...
function isAllowedOrigin(origin) {
  ...
  try {
    ...
    // Cho phép mọi preview deploy Netlify của đúng site (vd https://deploy-preview-12--xxx.netlify.app)
    // vẫn phải chứa đúng site id gốc để tránh match domain netlify.app bất kỳ.
    return h.endsWith('.netlify.app') && h.includes('blox-fruit-fake');
  } catch (err) { return false; }
}
// Hậu xử lý MỌI response trước khi trả về trình duyệt: thay Allow-Origin='*' bằng origin thật của request
// nếu nó nằm trong allow-list, hoặc BỎ HẲN header Allow-Origin nếu origin lạ/không có (an toàn hơn '*').
// Không có header Origin (app native, curl, server-to-server) -> giữ nguyên response, không cần header CORS.
function applyCors(request, response) {
  // FIX "không vào được phòng chung": phản hồi nâng cấp WebSocket (status 101, kèm field đặc biệt
  // `webSocket`) TUYỆT ĐỐI không được bọc lại bằng `new Response(...)` thông thường — runtime Cloudflare
  // Workers bắt buộc Response status 101 phải có kèm `webSocket`, nếu không sẽ ném lỗi ngay khi construct.
  // Trước đây applyCors() áp dụng vô điều kiện cho MỌI response (kể cả 101 của /room/:id/ws), nên mỗi lần
  // client cố mở WebSocket vào phòng chung, dòng `new Response(response.body, {...})` bên dưới ném lỗi
  // ngay lập tức -> request thất bại toàn bộ, client chỉ thấy kết nối bị đóng ngay (mã 1006) không rõ lý do.
  // WebSocket handshake vốn không cần và không dùng header CORS nên bỏ qua an toàn, trả nguyên response gốc.
...
// So sánh 2 chuỗi bí mật (token, admin key...) theo thời gian CỐ ĐỊNH — không được return/break sớm dựa
// theo độ dài hay vị trí ký tự sai đầu tiên, để không lộ thông tin qua độ trễ phản hồi (timing attack).
// Dùng thay cho `===` ở MỌI nơi so sánh token/API key nhạy cảm trong file này. Nếu độ dài khác nhau, vẫn
// chạy đủ vòng lặp theo độ dài chuỗi DÀI HƠN (so với chính nó ở phần thừa) để thời gian chạy không tỉ lệ
// thuận với việc "đoán đúng bao nhiêu ký tự đầu" — không rẽ nhánh sớm khi phát hiện length mismatch.
function timingSafeEqualStr(a, b) {
  const strA = typeof a === 'string' ? a : '';
  const strB = typeof b === 'string' ? b : '';
  const maxLen = Math.max(strA.length, strB.length);
  let diff = strA.length ^ strB.length; // độ dài khác nhau -> diff != 0 ngay, nhưng vẫn chạy đủ vòng lặp bên dưới
  for (let i = 0; i < maxLen; i++) {
    const codeA = i < strA.length ? strA.charCodeAt(i) : 0;
    const codeB = i < strB.length ? strB.charCodeAt(i) : 0;
    diff |= codeA ^ codeB;
  }
  return diff === 0 && strA.length > 0; // rỗng vs rỗng không coi là "khớp" hợp lệ (tránh bypass khi thiếu token)
}

// ============================================================
// RATE LIMITING — KV sliding-window đơn giản, không cần binding mới (dùng chung GAME_DATA đã có sẵn).
// Đếm số request theo (IP, endpoint) trong 1 cửa sổ thời gian cố định (fixed window, không phải sliding
// thật sự — đơn giản hơn, đủ dùng để chặn spam/brute-force, KHÔNG chính xác tuyệt đối ở biên cửa sổ, đó
// là đánh đổi chấp nhận được cho use-case này). Key KV: 'ratelimit:'+endpoint+':'+ip+':'+windowIndex,
// value = số lần gọi trong cửa sổ đó, TTL = đúng độ dài cửa sổ (tự dọn, không cần cron/cleanup riêng).
// LƯU Ý: đây là bảo vệ Ở MỨC ỨNG DỤNG (application-level) — không thay thế Cloudflare Rate Limiting Rules
// cấp mạng (nếu tài khoản Cloudflare có gói hỗ trợ, nên bật thêm ở Dashboard > Security > WAF cho phòng
// thủ theo lớp, vì lớp đó chặn TRƯỚC KHI request chạm tới Worker, đỡ tốn tài nguyên Worker/KV hơn).
//
// TỐI ƯU (giảm chính rate-limit gây tốn KV): trước đây MỖI lần gọi checkRateLimit() là 1 GET + (thường)
// 1 PUT KV — tức là cơ chế chống spam lại tự góp phần vào việc tốn quota KV. Nay thêm 1 lớp cache IN-MEMORY
// (module-level, sống trong 1 Worker isolate) giữ đúng bộ đếm hiện tại của (endpoint, ip, windowIndex) —
// chỉ GET KV lần ĐẦU TIÊN gặp 1 key trong cửa sổ đó, các lượt gọi tiếp theo trong CÙNG isolate + CÙNG cửa
// sổ chỉ tăng số trong RAM, không hỏi KV nữa. Vẫn PUT KV nền (không await) để đồng bộ nhẹ giữa các isolate
// khác nhau, nhưng số PUT giảm mạnh nhờ chỉ ghi khi số đếm đổi mốc đáng kể (xem WRITE_BACK_STEP bên dưới),
// không ghi lại KV ở MỌI request. Đánh đổi: giới hạn có thể lỏng hơn 1 chút giữa nhiều isolate khác nhau
// cùng lúc (mỗi isolate có bộ đếm RAM riêng, chỉ đồng bộ định kỳ) — chấp nhận được vì mục tiêu là chặn
// spam/brute-force, không phải giới hạn tài chính chính xác tuyệt đối.
const _rateLimitMemory = new Map(); // key -> { count, windowIndex, lastWrittenCount }
const RATE_LIMIT_WRITE_BACK_STEP = 5; // chỉ PUT KV lại sau mỗi 5 lần tăng trong RAM (giảm ~80% số PUT so với ghi mọi lần)

async function checkRateLimit(env, endpoint, ip, maxRequests, windowSeconds) {
  if (!ip || ip === 'unknown') return true; // không có IP để định danh -> không chặn được, cho qua (tránh chặn nhầm hàng loạt do lỗi đọc header)
  const windowIndex = Math.floor(Date.now() / (windowSeconds * 1000));
  const memKey = 'ratelimit:' + endpoint + ':' + ip;
  const kvKey = memKey + ':' + windowIndex;

  let entry = _rateLimitMemory.get(memKey);
  if (!entry || entry.windowIndex !== windowIndex) {
    // Cửa sổ mới (hoặc lần đầu gặp IP này) -> phải hỏi KV 1 lần để lấy số đếm hiện tại (có thể isolate
    // khác đã ghi trước đó), rồi từ đó dùng RAM cho các lượt gọi tiếp theo trong cùng cửa sổ.
    const currentRaw = await env.GAME_DATA.get(kvKey);
    const current = currentRaw ? parseInt(currentRaw, 10) || 0 : 0;
    entry = { count: current, windowIndex, lastWrittenCount: current };
    _rateLimitMemory.set(memKey, entry);
    // Dọn entry cũ khỏi Map định kỳ để không phình vô hạn (đơn giản: dọn khi Map quá lớn).
    if (_rateLimitMemory.size > 5000) {
      for (const [k, v] of _rateLimitMemory) { if (v.windowIndex !== windowIndex) _rateLimitMemory.delete(k); }
    }
  }
  ...
  // Chỉ PUT KV khi đã tăng đủ RATE_LIMIT_WRITE_BACK_STEP lần kể từ lần ghi trước, HOẶC khi vừa chạm mốc
  // giới hạn (để isolate khác kịp thấy giới hạn đã đầy, tránh vượt quá nhiều so với maxRequests thật).
  const stepsSinceWrite = entry.count - entry.lastWrittenCount;
  if (stepsSinceWrite >= RATE_LIMIT_WRITE_BACK_STEP || entry.count >= maxRequests) {
    entry.lastWrittenCount = entry.count;
    // Không await put() để không kéo dài thời gian phản hồi cho request hợp lệ — race condition nhẹ có thể
    // khiến giới hạn bị vượt vài request trong trường hợp cực hiếm (nhiều request đồng thời cùng đọc giá trị
    // cũ trước khi kịp ghi) — chấp nhận được vì đây là rate limit chống spam, không phải giới hạn tài chính.
    env.GAME_DATA.put(kvKey, String(entry.count), { expirationTtl: windowSeconds }).catch(() => {});
  }
  return true;
}

// Response chuẩn 429 khi vượt rate limit — dùng chung 1 định dạng cho mọi endpoint.
function rateLimitedResponse(retryAfterSeconds) {
  return new Response(JSON.stringify({ error: 'Quá nhiều yêu cầu, vui lòng thử lại sau ít phút.' }), {
    status: 429,
    headers: { 'Content-Type': 'application/json', 'Retry-After': String(retryAfterSeconds), ...CORS_HEADERS },
  });
}

// ============================================================
// GENERIC KV READ CACHE (in-memory, per isolate) — giảm số GET lặp lại cho các key ÍT ĐỔI, ĐỌC NHIỀU
// (meta:, tempadmin:, banned:*, group:, groups-of:, name:...) MÀ KHÔNG đụng tới bất kỳ đường ghi/xoá nào —
// mọi put()/delete() vẫn chạy y hệt cũ, cache module này chỉ RÚT NGẮN các lượt get() lặp lại trong cùng 1
// isolate, và LUÔN được xoá (invalidate) ngay khi có put()/delete() cho đúng key đó thông qua các hàm
// wrapper bên dưới — vì vậy không có rủi ro đọc dữ liệu cũ SAU KHI đã ghi thành công.
//
// Phân loại theo TTL (thời gian sống trong cache) dựa trên mức độ nhạy cảm nếu đọc phải giá trị cũ:
// - LONG (60s): dữ liệu gần như tĩnh trong 1 phiên chơi — meta info, group info, tempadmin, danh sách
//   groups-of. Đọc chậm vài chục giây không ảnh hưởng gameplay.
// - SHORT (5s): dữ liệu có thể đổi bất cứ lúc nào và ảnh hưởng bảo mật/trải nghiệm nếu stale quá lâu —
//   trạng thái banned/kicked. 5 giây là đủ ngắn để không tạo lỗ hổng "bị ban vẫn chơi được" kéo dài, vẫn
//   đủ dài để cắt phần lớn GET lặp lại (VD: WebSocket loop check kicked mỗi vài giây/người chơi).
// Bất cứ key nào không thuộc 2 nhóm trên (list() quét toàn bộ save, chat history, admin log...) KHÔNG
// được cache qua module này — luôn đọc trực tiếp KV để đảm bảo mới nhất, vì các nghiệp vụ đó vốn đã ít
// gọi hơn nhiều so với check ban/meta/group.
const KV_CACHE_TTL_LONG_MS = 60 * 1000;
const KV_CACHE_TTL_SHORT_MS = 5 * 1000;
const _kvReadCache = new Map(); // key -> { raw, ts, ttl }

function _kvCacheGetFresh(key) {
  const entry = _kvReadCache.get(key);
  if (!entry) return undefined; // chưa có trong cache -> phải hỏi KV
  if (Date.now() - entry.ts >= entry.ttl) { _kvReadCache.delete(key); return undefined; } // hết hạn -> phải hỏi KV lại
  return entry.raw; // có thể là null (nghĩa là "đã xác nhận KHÔNG tồn tại key này gần đây") — vẫn hợp lệ để dùng lại
}
function _kvCacheSet(key, raw, ttlMs) {
  _kvReadCache.set(key, { raw, ts: Date.now(), ttl: ttlMs });
  // Dọn cache định kỳ để không phình vô hạn theo thời gian server chạy.
  if (_kvReadCache.size > 8000) {
    const now = Date.now();
    for (const [k, v] of _kvReadCache) { if (now - v.ts >= v.ttl) _kvReadCache.delete(k); }
  }
}
// Xoá 1 key khỏi cache — BẮT BUỘC gọi ngay sau mọi put()/delete() cho key đó, để lượt đọc kế tiếp (dù
// trong cùng isolate hay không) không bao giờ thấy giá trị cũ hơn lượt ghi vừa thành công.
function _kvCacheInvalidate(key) {
  _kvReadCache.delete(key);
}

// GET có cache — dùng cho các key ÍT NHẠY CẢM nếu stale (TTL dài, mặc định 60s). KHÔNG dùng cho dữ liệu
// cần chính xác tuyệt đối theo thời gian thực (VD: save chính, số dư gold) — những chỗ đó vẫn phải đọc
// trực tiếp env.GAME_DATA.get() hoặc qua readSaveRecord() như cũ (đã có buffer riêng, không đụng ở đây).
async function cachedKvGet(env, key, ttlMs = KV_CACHE_TTL_LONG_MS) {
  const cached = _kvCacheGetFresh(key);
  if (cached !== undefined) return cached;
  const raw = await env.GAME_DATA.get(key);
  _kvCacheSet(key, raw, ttlMs);
  return raw;
}
// PUT + tự invalidate cache — dùng thay cho env.GAME_DATA.put() ở các key đang được đọc qua cachedKvGet().
async function cachedKvPut(env, key, value, options) {
  const result = await env.GAME_DATA.put(key, value, options);
  _kvCacheInvalidate(key);
  return result;
}
// DELETE + tự invalidate cache — dùng thay cho env.GAME_DATA.delete() ở các key đang được đọc qua cachedKvGet().
async function cachedKvDelete(env, key) {
  const result = await env.GAME_DATA.delete(key);
  _kvCacheInvalidate(key);
  return result;
}
...
async function isProtectedAdmin(env, name) {
  ...
  const tempRaw = await cachedKvGet(env, 'tempadmin:' + name, KV_CACHE_TTL_LONG_MS);
  return !!tempRaw;
}
...
async function recordPlayerMeta(env, name, request, deviceId) {
  ...
  if (!deviceId) {
    try {
      const prevRaw = await cachedKvGet(env, 'meta:' + name, KV_CACHE_TTL_LONG_MS);
  ...
  await cachedKvPut(env, 'meta:' + name, JSON.stringify(meta));
}
...
// TTL ngắn (5s) cho các key banned:* — đây là kiểm tra bảo mật, không nên cache quá lâu (chấp nhận trễ
// tối đa 5s giữa lúc admin ban và lúc lệnh cấm có hiệu lực hoàn toàn, đổi lại giảm mạnh GET lặp lại vì
// checkBanned() chạy ở HẦU HẾT mọi request cần xác thực).
async function checkBanned(env, name, request, deviceId) {
  if (name && await isProtectedAdmin(env, name)) return null;
  if (name && await cachedKvGet(env, 'banned:' + name, KV_CACHE_TTL_SHORT_MS)) return 'Tài khoản "' + name + '" đã bị cấm.';
  const ip = getClientIp(request);
  if (ip && ip !== 'unknown' && await cachedKvGet(env, 'banned-ip:' + ip, KV_CACHE_TTL_SHORT_MS)) return 'Thiết bị/IP này đã bị cấm.';
  if (deviceId && await cachedKvGet(env, 'banned-device:' + deviceId, KV_CACHE_TTL_SHORT_MS)) return 'Thiết bị này đã bị cấm.';
  return null;
}
...
/* ============================================================
   HỢP NHẤT ĐỌC/GHI 3 KEY (save: / meta: / name:) --------------------------------------------------------
   KHÔNG đổi cấu trúc 3 key hiện có (không migrate KV cũ) — module này chỉ gộp THAO TÁC I/O: đọc cả 3 key
   SONG SONG 1 lần bằng Promise.all (thay vì rải rác nhiều lượt get() độc lập ở verifyAccountToken/
   writeSaveRecord/recordPlayerMeta như trước), gộp phần data cần sửa trên bộ nhớ, rồi GHI cả 3 lại trong
   ĐÚNG 1 Promise.all cuối cùng — giảm số round-trip KV cho endpoint /save từ ~4-5 lượt (get name, có thể
   put name khi claim token, get meta cũ, put save, put meta) xuống còn 2 lượt (1 đọc gộp, 1 ghi gộp).
   Dữ liệu save (`save:`) vẫn luôn được mã hoá AES-GCM qua aesGcmEncryptToJson/readSaveRecord-compatible
   logic; `meta:`/`name:` giữ nguyên JSON thường như trước (không nằm trong phạm vi yêu cầu mã hoá).
============================================================ */
// Đọc gộp 3 bản ghi liên quan tới 1 tên tài khoản, song song. Tự giải mã save nếu là envelope AES-GCM,
// tương thích ngược với bản ghi save chưa mã hoá. Trả về { saveData, metaData, nameRecord } (mỗi field
// null nếu key tương ứng chưa tồn tại).
async function readMergedPlayerRecord(env, name) {
  const [saveRaw, metaRaw, nameRaw] = await Promise.all([
    env.GAME_DATA.get('save:' + name), // save: có buffer riêng ở writeSaveRecord/readSaveRecord, không cache thêm ở đây
    cachedKvGet(env, 'meta:' + name, KV_CACHE_TTL_LONG_MS),
    cachedKvGet(env, 'name:' + name, KV_CACHE_TTL_SHORT_MS), // name: gắn với token xác thực -> TTL ngắn cho an toàn
  ]);
  let saveData = null;
  if (saveRaw !== null && saveRaw !== undefined) {
    try {
      saveData = looksLikeAesEnvelope(saveRaw) ? await aesGcmDecryptFromJson(env, saveRaw) : JSON.parse(saveRaw);
    } catch (err) { saveData = null; }
  }
  let metaData = null;
  if (metaRaw) { try { metaData = JSON.parse(metaRaw); } catch (err) { metaData = null; } }
  let nameRecord = null;
  if (nameRaw) { try { nameRecord = JSON.parse(nameRaw); } catch (err) { nameRecord = null; } }
  return { saveData, metaData, nameRecord };
}

// Ghi gộp 3 bản ghi trong ĐÚNG 1 Promise.all — chỉ ghi những field được truyền vào (undefined = bỏ qua,
// giữ nguyên bản KV hiện có, không ghi đè bằng rỗng). saveData luôn được mã hoá AES-GCM trước khi put.
async function writeMergedPlayerRecord(env, name, { saveData, metaData, nameRecord } = {}) {
  const writes = [];
  if (saveData !== undefined) {
    writes.push(aesGcmEncryptToJson(env, saveData).then(enc => env.GAME_DATA.put('save:' + name, enc)));
  }
  if (metaData !== undefined) {
    writes.push(cachedKvPut(env, 'meta:' + name, JSON.stringify(metaData)));
  }
  if (nameRecord !== undefined) {
    writes.push(cachedKvPut(env, 'name:' + name, JSON.stringify(nameRecord)));
  }
...
async function verifyAccountToken(env, name, token, deviceId, allowClaim) {
  const raw = await cachedKvGet(env, 'name:' + name, KV_CACHE_TTL_SHORT_MS);
  if (!raw) throw new Error('Tài khoản "' + name + '" chưa được đăng ký.');
  let record;
  try { record = JSON.parse(raw); } catch (err) { record = {}; }
  if (record.token) {
    if (token && timingSafeEqualStr(token, record.token)) return record;
    throw new Error('Xác thực thất bại — đây không phải tài khoản của bạn.');
  }
  // Tài khoản cũ chưa có token
  if (allowClaim && token) {
    record.token = token;
    await cachedKvPut(env, 'name:' + name, JSON.stringify(record));
    return record;
  }
  const metaRaw = await cachedKvGet(env, 'meta:' + name, KV_CACHE_TTL_LONG_MS);
...
/* ============================================================
   EXPORT / IMPORT TÀI KHOẢN — cấp 1 "mã export" (export code) người chơi có thể copy sang máy/trình
   duyệt khác để load lại đúng save của mình, KHÔNG cần đăng nhập bằng token cũ lưu localStorage.

   Luồng v4 — MÃ HOÁ AES-GCM (IV ngẫu nhiên) + KÝ HMAC + HẾT HẠN + DÙNG 1 LẦN:
     key    = HKDF-SHA256(EXPORT_AES_KEY) dẫn xuất riêng cho mục đích export (độc lập khoá ký HMAC)
     iv     = 12 byte ngẫu nhiên MỚI mỗi lần export (không bao giờ tái sử dụng)
     cipher = AES-GCM-Encrypt(key, iv, JSON.stringify(data))   -> data không bao giờ nằm dạng chữ rõ trong mã
     ticket = { accountId, ts, nonce, iv, cipher }
     signed = HMAC-SHA256( accountId + ':' + ts + ':' + nonce + ':' + iv + ':' + cipher )  ký bằng EXPORT_SIGN_KEY
     EXPORT CODE = base64url(JSON.stringify({ accountId, ts, nonce, iv, cipher, signed }))
   Verify: tính lại HMAC trên đúng {accountId, ts, nonce, iv, cipher} lấy từ code -> so với signed đi kèm,
   RỒI mới giải mã AES-GCM. Vì cả 2 lớp (mã hoá + chữ ký) đều dùng secret CHỈ SERVER BIẾT nên:
   (1) client không tự chế mã giả cho tên người khác, (2) ai sửa tay dù 1 byte trong `cipher`/`iv` sau khi
   export (kể cả không đọc được nội dung để biết sửa gì) cũng khiến signed không khớp -> bị từ chối ngay,
   VÀ AES-GCM tự thân đã có cơ chế xác thực toàn vẹn (auth tag) nên giải mã cũng sẽ thất bại độc lập với
   HMAC — hai lớp bảo vệ tách biệt ("chống sửa code ✅✅").
   Các secret export (EXPORT_SIGN_KEY/EXPORT_AES_KEY) KHÔNG BAO GIỜ gửi xuống client/HTML — chỉ chạy trên
   Worker; client chỉ thấy chuỗi base64url đã mã hoá, không tự giải mã được.
   Import CHỈ CẦN gọi server để verify (không cần server tồn tại KV save cho tên đó nữa) — do đó vẫn cần
   mạng (để verify), nhưng KHÔNG còn phụ thuộc server còn lưu save hay không.
   DÙNG 1 LẦN: `nonce` (16 byte ngẫu nhiên, không đoán được) là "chìa khoá" duy nhất cho mỗi mã. Sau khi
   1 mã được import THÀNH CÔNG, server ghi 'used-export:'+nonce vào KV (TTL 1 năm — đủ dài để coi như
   vĩnh viễn, tránh giữ KV vô hạn không cần thiết). Lần import kế tiếp với CÙNG mã đó sẽ bị từ chối ngay
   ở bước kiểm tra KV, TRƯỚC KHI làm bất kỳ việc gì khác — dù chữ ký vẫn đúng. Vì nonce ngẫu nhiên và
   không xuất hiện lại giữa các lần export khác nhau, đánh dấu 1 nonce không ảnh hưởng gì tới các mã
   export khác (kể cả export nhiều lần liên tiếp cho cùng 1 tài khoản).
   HẾT HẠN (EXPIRATION): mỗi mã chỉ có hiệu lực EXPORT_CODE_MAX_AGE_MS kể từ `ts` lúc tạo — quá hạn bị từ
   chối ngay cả khi chưa từng dùng, để hạn chế rủi ro nếu mã bị lộ (chat, ảnh chụp màn hình...) lâu ngày.
   LỚP BỌC NGOÀI (thứ 3): toàn bộ chuỗi trên còn được bọc thêm 1 lớp HMAC-SHA256 NGOÀI CÙNG bằng secret
   EXPORT_WRAP_KEY — secret này ĐỘC LẬP với ADMIN_KEY/EXPORT_SIGN_KEY/EXPORT_AES_KEY (không dẫn xuất từ nhau, không có mặt trong bất kỳ
   file code nào, chỉ tồn tại dưới dạng Cloudflare secret set qua `wrangler secret put EXPORT_WRAP_KEY`).
   Xem chi tiết + lý do (defense in depth) ngay phía trên hàm signOuterWrap/wrapOuterLayer/unwrapOuterLayer.
============================================================ */
const EXPORT_CODE_MAX_AGE_MS = 24 * 60 * 60 * 1000; // 24 giờ — đủ để chuyển máy nhưng không để mã cũ tồn tại vô hạn nếu bị lộ.

/* ---- LỚP BỌC NGOÀI CÙNG: HMAC-SHA256 với secret RIÊNG "EXPORT_WRAP_KEY" -------------------------------
   Đây là secret ĐỘC LẬP HOÀN TOÀN với ADMIN_KEY/EXPORT_SIGN_KEY/EXPORT_AES_KEY — không dẫn xuất từ nhau, không lưu trong
   bất kỳ file code nào (worker.js lẫn index.html đều KHÔNG chứa giá trị của nó, chỉ chứa TÊN biến env để
   đọc ra). Phải set bằng lệnh (server tự lưu trong Cloudflare, tách biệt khỏi source code):
       wrangler secret put EXPORT_WRAP_KEY
   Mục đích: phòng thủ theo lớp (defense in depth) — nếu EXPORT_SIGN_KEY/EXPORT_AES_KEY (dùng để ký + mã hoá AES-GCM ở trên) vì
   lý do nào đó bị lộ, mã export VẪN còn bị chặn bởi lớp bọc ngoài này vì nó dùng secret hoàn toàn khác mà
   kẻ tấn công không có. Bọc SAU CÙNG (ngoài cùng chuỗi base64url cuối cùng người dùng copy/paste) và kiểm
   tra ĐẦU TIÊN lúc verify — sai lớp bọc ngoài thì bị từ chối ngay, không cần đụng tới AES-GCM/HMAC bên
   trong (accountId, ts, cipher...) — kẻ tấn công không có EXPORT_WRAP_KEY thì không thể tự tạo ra 1 mã
   "coi được" để server chịu bóc lớp trong ra thử. ---------------------------------------------------- */
/* ---- MỤC 7/8/9 (hardening): VERSIONED KEY RING cho lớp bọc ngoài cùng --------------------------------
   Trước đây chỉ có 1 secret EXPORT_WRAP_KEY + 1 EXPORT_WRAP_KEY_PREV, và ticket không mang "kid" — khi
   giải mã server phải THỬ TUẦN TỰ từng secret ứng viên (kiểu brute-force nội bộ), vi phạm nguyên tắc
   "không brute-force nếu kid đã xác định được key". Nay chuyển sang key ring có định danh (kid) rõ ràng:
     EXPORT_WRAP_KEY_CURRENT  (kid 'wrap-current') -> DUY NHẤT được dùng để tạo export MỚI.
     EXPORT_WRAP_KEY_PREV_1   (kid 'wrap-prev-1')  -> chỉ dùng để ĐỌC export cũ trong migration window.
     EXPORT_WRAP_KEY_PREV_2   (kid 'wrap-prev-2')  -> chỉ dùng để ĐỌC export cũ hơn nữa.
     EXPORT_WRAP_KEY          (kid 'wrap-legacy')  -> secret CŨ (trước khi có key ring), giữ lại CHỈ để đọc
                                                       ticket v4 cũ (không có field kid) phát hành trước đây.
   kid CHỈ là identifier để server tra map -> secret tương ứng, KHÔNG phải bản thân secret, và client gửi
   kid lên không được tin tưởng mù quáng — server tự kiểm tra kid có nằm trong danh sách hợp lệ/còn hiệu
   lực (không REVOKED) trước khi dùng để tra key.
   Rotation: set EXPORT_WRAP_KEY_PREV_2 = giá trị cũ của PREV_1, EXPORT_WRAP_KEY_PREV_1 = giá trị cũ của
   CURRENT, rồi set EXPORT_WRAP_KEY_CURRENT = secret mới, deploy. Giữ tối đa CURRENT + 2 PREV; secret nào
   rơi khỏi 3 vị trí này thì coi là REVOKED (xoá biến env, ticket ký bằng key đó sẽ bị từ chối ngay ở bước
   tra kid — không còn "brute-force" thử nó nữa). ---------------------------------------------------- */
const EXPORT_WRAP_KEY_RING = [
  { kid: 'wrap-current', envName: 'EXPORT_WRAP_KEY_CURRENT', status: 'CURRENT' },
  { kid: 'wrap-prev-1', envName: 'EXPORT_WRAP_KEY_PREV_1', status: 'PREV' },
  { kid: 'wrap-prev-2', envName: 'EXPORT_WRAP_KEY_PREV_2', status: 'PREV' },
  // Legacy: secret cũ trước khi có key ring (ticket v4 cũ không có field `kid`) — CHỈ đọc, không bao giờ
  // dùng để ký export mới. Có thể xoá khỏi ring này (và khỏi Cloudflare secrets) sau migration window khi
  // chắc chắn không còn export code cũ nào (>24h kể từ khi ngừng phát hành) cần đọc nữa.
  { kid: 'wrap-legacy', envName: 'EXPORT_WRAP_KEY', status: 'PREV' },
];
// Secret DUY NHẤT dùng để ký/tạo export MỚI — luôn là CURRENT, không bao giờ PREV.
function currentWrapKeyEntry(env) {
  const entry = EXPORT_WRAP_KEY_RING[0]; // 'wrap-current'
  const secret = env[entry.envName];
  if (!secret) throw new Error('Server chưa cấu hình khoá bọc ngoài hiện tại (EXPORT_WRAP_KEY_CURRENT).');
  return { kid: entry.kid, secret };
}
// Tra secret theo kid — CHỈ chấp nhận kid có trong ring VÀ secret env tương ứng đã được set (còn hiệu
// lực); kid lạ hoặc secret đã bị xoá khỏi env (coi như REVOKED) đều bị từ chối ngay, không thử key khác.
function resolveWrapKeyByKid(env, kid) {
  const entry = EXPORT_WRAP_KEY_RING.find(e => e.kid === kid);
  if (!entry) return null;
  const secret = env[entry.envName];
  if (!secret) return null; // secret đã bị revoke/chưa cấu hình
  return secret;
}
...
// Ký lớp bọc ngoài bằng key CURRENT — trả về cả kid (để nhúng vào ticket) và chữ ký.
async function signOuterWrap(env, payload) {
  const { kid, secret } = currentWrapKeyEntry(env);
  const wrap = await hmacSha256(secret, payload);
  return { kid, wrap };
}
// Bọc 1 mã (chuỗi base64url của ticket AES-GCM+HMAC bên trong) thêm 1 lớp HMAC-SHA256 ngoài cùng, có kid.
async function wrapOuterLayer(env, innerCode) {
  const { kid, wrap } = await signOuterWrap(env, innerCode);
  return textToBase64Url(JSON.stringify({ p: innerCode, w: wrap, kid }));
}
// Bóc lớp bọc ngoài — verify HMAC TRƯỚC KHI đụng tới bất kỳ phần nào của ticket bên trong.
// kid quyết định server tra secret nào (không brute-force toàn bộ ring); ticket cũ (trước khi có kid)
// mặc định thử key 'wrap-legacy' để không phá các mã export đã phát hành trước migration.
async function unwrapOuterLayer(env, code) {
  let outer;
  try {
    outer = JSON.parse(base64UrlToText(String(code || '').trim()));
  } catch (err) {
    throw new Error('Mã import không hợp lệ (sai định dạng lớp bọc ngoài).');
  }
  const { p, w } = outer || {};
  const kid = typeof outer?.kid === 'string' && outer.kid ? outer.kid : 'wrap-legacy';
  if (!p || typeof p !== 'string' || !w || typeof w !== 'string') {
    throw new Error('Mã import thiếu lớp bọc bảo vệ ngoài cùng (có thể là mã kiểu cũ trước khi có lớp bọc — hãy Xuất lại mã mới).');
  }
  const secret = resolveWrapKeyByKid(env, kid);
  if (!secret) throw new Error('Mã import không hợp lệ (khoá ký đã hết hiệu lực hoặc không xác định — hãy Xuất lại mã mới).');
  const expected = await hmacSha256(secret, p);
  if (expected.length !== w.length) throw new Error('Mã import không hợp lệ (sai lớp bọc ngoài) — có thể mã đã bị sửa hoặc không phải do server này cấp.');
  let diff = 0;
  for (let i = 0; i < expected.length; i++) diff |= expected.charCodeAt(i) ^ w.charCodeAt(i);
  if (diff !== 0) throw new Error('Mã import không hợp lệ (sai lớp bọc ngoài) — có thể mã đã bị sửa hoặc không phải do server này cấp.');
  return p;
}

// Chuyển ArrayBuffer -> base64url (không padding '=', đổi '+/' thành '-_') để nhét gọn vào 1 chuỗi text
// người dùng copy/paste được, không lo ký tự đặc biệt bị mất khi dán vào ô input/chat.
...
// Text (UTF-8) -> base64url và ngược lại, dùng để đóng gói cả object ticket JSON vào 1 chuỗi export code.
function textToBase64Url(str) {
  return bufToBase64Url(new TextEncoder().encode(str));
}
function base64UrlToText(str) {
  return new TextDecoder().decode(base64UrlToBuf(str));
}
// Sinh 1 nonce ngẫu nhiên 16 byte (128-bit, không đoán được) -> base64url, dùng làm định danh DUY NHẤT
// cho mỗi mã export — đây là "chìa khoá" để đánh dấu 1 mã cụ thể đã được dùng hay chưa trong KV.
function genExportNonce() {
  return bufToBase64Url(crypto.getRandomValues(new Uint8Array(16)));
}

// Chuỗi dùng làm input ký/verify HMAC — GHÉP CỐ ĐỊNH accountId + ts + nonce + iv + cipher, để bất kỳ
// thay đổi nào ở cipher (dù chỉ 1 byte) hoặc ở nonce/iv đều làm input đổi -> chữ ký không khớp nữa.
// Lưu ý: ký trên CIPHER (đã mã hoá) chứ không phải trên `data` thô, vì `data` không còn xuất hiện dạng
// chữ rõ trong ticket nữa (đã được AES-GCM mã hoá) — xem AES_GCM helpers bên dưới.
function exportTicketMessage(accountId, ts, nonce, iv, cipher) {
  return accountId + ':' + ts + ':' + nonce + ':' + iv + ':' + cipher;
}

// Ký HMAC-SHA256 bằng secret RIÊNG "EXPORT_SIGN_KEY" — ĐỘC LẬP HOÀN TOÀN với ADMIN_KEY (ADMIN_KEY giờ
// CHỈ dùng cho /admin/*, không còn liên quan gì tới export/import). Set bằng: wrangler secret put EXPORT_SIGN_KEY
// Trả về base64url của chữ ký — không phải hex, để mã export ngắn gọn hơn.
async function signExportTicket(env, accountId, ts, nonce, iv, cipher) {
  if (!env.EXPORT_SIGN_KEY) throw new Error('Server chưa cấu hình khoá ký (EXPORT_SIGN_KEY).');
  const key = await crypto.subtle.importKey(
    'raw', new TextEncoder().encode(env.EXPORT_SIGN_KEY),
    { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const sig = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(exportTicketMessage(accountId, ts, nonce, iv, cipher)));
  return bufToBase64Url(sig);
}

// Dẫn xuất khoá AES-256-GCM từ secret RIÊNG "EXPORT_AES_KEY" — ĐỘC LẬP HOÀN TOÀN với ADMIN_KEY và với
// EXPORT_SIGN_KEY (3 secret riêng biệt, không secret nào dẫn xuất từ secret khác).
// Set bằng: wrangler secret put EXPORT_AES_KEY
async function deriveExportAesKey(env) {
  if (!env.EXPORT_AES_KEY) throw new Error('Server chưa cấu hình khoá mã hoá (EXPORT_AES_KEY).');
  const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(env.EXPORT_AES_KEY), 'HKDF', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: new TextEncoder().encode('export-aes-gcm-v4') },
    baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
  );
}

// Mã hoá `data` bằng AES-GCM với IV NGẪU NHIÊN 12 byte MỚI mỗi lần gọi (không bao giờ tái sử dụng IV với
// cùng 1 khoá — đây là điều kiện bắt buộc để AES-GCM an toàn). Trả về { iv, cipher } dạng base64url.
async function encryptExportData(env, data) {
  const key = await deriveExportAesKey(env);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const plaintext = new TextEncoder().encode(JSON.stringify(data === undefined ? null : data));
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plaintext);
  return { iv: bufToBase64Url(iv.buffer), cipher: bufToBase64Url(cipherBuf) };
}

// Giải mã cipher AES-GCM về lại object `data` gốc. AES-GCM tự có auth tag nên nếu cipher/iv bị sửa dù 1
// byte, subtle.decrypt sẽ NÉM LỖI (không âm thầm trả về rác) — đây là lớp bảo vệ THỨ HAI độc lập với HMAC.
async function decryptExportData(env, ivB64, cipherB64) {
  const key = await deriveExportAesKey(env);
  const iv = new Uint8Array(base64UrlToBuf(ivB64));
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, key, base64UrlToBuf(cipherB64));
  return JSON.parse(new TextDecoder().decode(plainBuf));
}

/* ============================================================
   LỚP BỌC AES-GCM DÙNG CHUNG (save data + export ticket) -----------------------------------------------
   Module này ĐỘC LẬP với deriveExportAesKey/encryptExportData/decryptExportData ở trên (khoá đó dẫn xuất
   từ ADMIN_KEY, dùng riêng cho phần "data nhúng trong export ticket" — giữ nguyên, KHÔNG đổi để không phá
   các mã export cũ đã phát ra). Module MỚI này dẫn xuất khoá từ secret RIÊNG EXPORT_WRAP_KEY (đã có sẵn
   trong env, vốn trước đây chỉ dùng để ký HMAC lớp bọc ngoài cùng — xem signOuterWrap ở trên) bằng
   HKDF-SHA256 với `info` cố định riêng, để tạo ra 1 khoá AES-256-GCM tách biệt hoàn toàn khỏi khoá HMAC
   dùng chung secret đó.
   PHẠM VI SỬ DỤNG:
     1) Mã hoá TOÀN BỘ save data khi ghi xuống KV ('save:'+name) và giải mã khi đọc lên — dữ liệu account
        không còn nằm dạng JSON chữ rõ trong KV nữa, phòng trường hợp KV bị truy cập trái phép (vd lộ
        Cloudflare API Token chỉ có quyền đọc KV mà không có ADMIN_KEY/EXPORT_WRAP_KEY).
     2) Mã hoá thêm 1 lớp AES-GCM ở NGOÀI CÙNG (bọc quanh chuỗi wrapOuterLayer đã ký HMAC) cho export code,
        dùng CHUNG khoá dẫn xuất từ EXPORT_WRAP_KEY — tận dụng đúng secret đã ghi rõ trong comment đầu file
        là "lớp bọc ngoài cùng" cho mã export/import.
   ĐỊNH DẠNG LƯU TRONG KV: thay vì lưu thẳng JSON.stringify(data), giờ lưu JSON.stringify({ v: 1, iv, cipher })
   — có version `v` để về sau có thể đổi thuật toán/khoá mà vẫn nhận diện được bản ghi cũ nếu cần migrate.
   TƯƠNG THÍCH NGƯỢC: readSaveRecord() bên dưới tự phát hiện bản ghi CHƯA mã hoá (JSON thường, không có
   field `cipher`) và trả về nguyên trạng — cho phép rollout dần dần mà không cần migrate toàn bộ KV ngay,
   nhưng mọi lần GHI MỚI đều luôn ở dạng đã mã hoá (write-through encrypt).
============================================================ */
/* ============================================================
   LỚP MÃ HOÁ RIÊNG CHO SAVE DATA — secret "SAVE_DATA_KEY" ------------------------------------------------
   TRƯỚC ĐÂY: save data ('save:'+name) dùng CHUNG khoá dẫn xuất từ EXPORT_WRAP_KEY (khoá vốn dành cho lớp
   bọc ngoài cùng của export/import) — gộp 2 mục đích khác nhau vào 1 secret. NAY: tách hẳn secret RIÊNG
   "SAVE_DATA_KEY" — ĐỘC LẬP HOÀN TOÀN với EXPORT_WRAP_KEY/EXPORT_SIGN_KEY/EXPORT_AES_KEY/ADMIN_KEY/
   ADMIN_PROTECT_SECRET (không dẫn xuất từ nhau) — CHỈ DÙNG để mã hoá/giải mã save data trong KV.
   Set bằng: wrangler secret put SAVE_DATA_KEY
   TƯƠNG THÍCH NGƯỢC (2 tầng):
     1) Nếu server CHƯA set SAVE_DATA_KEY (chưa kịp `wrangler secret put`), ghi mới TẠM THỜI vẫn dùng
        EXPORT_WRAP_KEY như cũ (fail-open có chủ đích, giống ADMIN_PROTECT_SECRET) — không khoá cứng ghi
        save của người chơi trước khi admin kịp cấu hình secret mới.
     2) Giải mã: thử SAVE_DATA_KEY trước (nếu có) -> SAVE_DATA_KEY_PREV (rotation) -> EXPORT_WRAP_KEY (dữ
        liệu mã hoá TỪ TRƯỚC KHI có SAVE_DATA_KEY) -> EXPORT_WRAP_KEY_PREV — đảm bảo save cũ vẫn đọc được
        trong suốt quá trình chuyển đổi, không mất dữ liệu người chơi.
   KEY ROTATION cho SAVE_DATA_KEY (khi nghi lộ / đổi định kỳ):
     1. wrangler secret put SAVE_DATA_KEY_PREV   -> đặt = khoá CŨ hiện tại
     2. wrangler secret put SAVE_DATA_KEY        -> đặt = khoá MỚI
     3. Deploy lại. GHI MỚI luôn dùng khoá MỚI; GIẢI MÃ tự thử lần lượt như trên.
     KHÔNG xoá SAVE_DATA_KEY_PREV/EXPORT_WRAP_KEY ngay — làm vậy có thể khiến save CŨ không giải mã được.
============================================================ */
async function deriveSaveAesKey(env, secretOverride) {
  const secret = secretOverride;
  if (!secret) throw new Error('Server chưa cấu hình khoá mã hoá dữ liệu.');
  const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), 'HKDF', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: new TextEncoder().encode('save-data-aes-gcm-v1') },
    baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
  );
}
// Thứ tự ứng viên secret dùng để GHI MỚI: SAVE_DATA_KEY (chuẩn mới) -> EXPORT_WRAP_KEY (fallback tạm thời
// nếu chưa set SAVE_DATA_KEY) — luôn lấy CANDIDATE ĐẦU TIÊN có giá trị, không bao giờ dùng khoá *_PREV để ghi mới.
function currentSaveDataSecret(env) {
  return env.SAVE_DATA_KEY || env.EXPORT_WRAP_KEY || null;
}
// Toàn bộ chuỗi ứng viên dùng để THỬ GIẢI MÃ, theo đúng thứ tự ưu tiên mô tả ở trên. Lọc bỏ giá trị rỗng/trùng.
function saveDataDecryptCandidates(env) {
  const list = [env.SAVE_DATA_KEY, env.SAVE_DATA_KEY_PREV, env.EXPORT_WRAP_KEY, env.EXPORT_WRAP_KEY_PREV];
  return [...new Set(list.filter(v => typeof v === 'string' && v))];
}

// Mã hoá 1 object bất kỳ (save data) bằng AES-GCM, IV ngẫu nhiên 12 byte mới mỗi lần gọi. Trả về chuỗi
// JSON string sẵn sàng để `put` thẳng vào KV. LUÔN mã hoá bằng secret HIỆN TẠI (currentSaveDataSecret) —
// không bao giờ dùng khoá *_PREV để ghi mới; rotation chỉ ảnh hưởng chiều GIẢI MÃ (xem hàm dưới).
async function aesGcmEncryptToJson(env, data) {
  const secret = currentSaveDataSecret(env);
  if (!secret) throw new Error('Server chưa cấu hình khoá mã hoá dữ liệu (SAVE_DATA_KEY).');
  const key = await deriveSaveAesKey(env, secret);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const plaintext = new TextEncoder().encode(JSON.stringify(data === undefined ? null : data));
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plaintext);
  return JSON.stringify({ v: 1, iv: bufToBase64Url(iv.buffer), cipher: bufToBase64Url(cipherBuf) });
}

async function aesGcmDecryptFromJson(env, jsonStr) {
  let envelope;
  try {
    envelope = JSON.parse(jsonStr);
  } catch (err) {
    throw new Error('Dữ liệu mã hoá không hợp lệ (sai định dạng JSON).');
  }
  const { iv, cipher } = envelope || {};
  if (!iv || typeof iv !== 'string' || !cipher || typeof cipher !== 'string') {
    throw new Error('Dữ liệu mã hoá không hợp lệ (thiếu iv/cipher).');
  }
  const ivBytes = new Uint8Array(base64UrlToBuf(iv));
  const cipherBytes = base64UrlToBuf(cipher);
  const candidates = saveDataDecryptCandidates(env);
  if (candidates.length === 0) throw new Error('Server chưa cấu hình khoá giải mã dữ liệu (SAVE_DATA_KEY).');
  let lastErr;
...
// Kiểm tra nhanh 1 chuỗi JSON đọc từ KV có phải là envelope đã mã hoá bởi aesGcmEncryptToJson() hay không
// (dùng để phân biệt bản ghi cũ chưa mã hoá, cho phép tương thích ngược trong readSaveRecord/writeSaveRecord).
function looksLikeAesEnvelope(jsonStr) {
  if (!jsonStr || typeof jsonStr !== 'string') return false;
  try {
    const obj = JSON.parse(jsonStr);
    return !!(obj && typeof obj === 'object' && obj.v === 1 && typeof obj.iv === 'string' && typeof obj.cipher === 'string');
  } catch (err) {
    return false;
  }
}

// ==================== SAVE-RECORD BUFFER (giảm KV read/write) ====================
// Mục tiêu: nhiều hành động liên tiếp của CÙNG 1 người chơi trong vài giây (VD: ăn nhiều đòn liên tục
// trong PvP, nhặt loot dồn dập...) trước đây MỖI hành động gọi writeSaveRecord() -> 1 lượt PUT KV riêng.
// KV free tier chỉ có 1.000 PUT/ngày -> rất dễ cháy quota với game realtime. Giải pháp: gom (debounce)
// các lần ghi liên tiếp của cùng 1 tên trong 1 cửa sổ ngắn thành ĐÚNG 1 lượt PUT, và cache bản đọc gần
// nhất trong bộ nhớ Worker (module-level, sống trong 1 isolate) để đọc lặp lại không cần hỏi KV lại ngay.
//
// LƯU Ý QUAN TRỌNG VỀ ĐỘ AN TOÀN DỮ LIỆU:
// - Bộ nhớ Worker (isolate) có thể bị Cloudflare giải phóng bất cứ lúc nào (không đảm bảo tồn tại lâu
//   dài như Durable Object storage) -> KHÔNG dùng buffer này để trì hoãn vô thời hạn. Cửa sổ gom mặc
//   định chỉ SAVE_BUFFER_DELAY_MS (vài giây), và luôn có ctx.waitUntil() đảm bảo lượt flush cuối cùng
//   được chạy xong dù response đã trả về client.
// - Với các nghiệp vụ mang tính TOÀN VẸN cao (trade, bán item, đổi tên, export/import, admin command) —
//   nơi mất 1 lần ghi là không chấp nhận được — LUÔN gọi writeSaveRecord(env, name, data, { forceFlush:
//   true }) để ghi thẳng xuống KV ngay lập tức, bỏ qua buffer hoàn toàn.
// - Mỗi Durable Object / Worker instance có buffer RIÊNG (không chia sẻ giữa các instance khác nhau),
//   nên 2 request cho CÙNG 1 account nhưng xử lý ở 2 DO instance khác nhau vẫn có thể ghi đè nhau như
//   trước đây — buffer này KHÔNG thay thế cơ chế serialize của getAccountLockStub, chỉ giảm số lượt PUT
//   trong phạm vi 1 instance đang xử lý dồn dập nhiều hành động nhỏ liên tiếp (VD: trong 1 DO GuildState
//   xử lý nhiều đòn đánh arena liên tục của cùng 1 người).
const SAVE_BUFFER_DELAY_MS = 6000; // gom tối đa 6 giây trước khi ghi thật xuống KV
const _saveWriteBuffer = new Map(); // name -> { data, timer, pendingFlush(Promise) }
const _saveReadCache = new Map();   // name -> { data, ts } — cache đọc ngắn hạn, tự hết hạn theo SAVE_BUFFER_DELAY_MS

// Ghi THẬT xuống KV ngay lập tức (mã hoá trước khi put) — dùng nội bộ bởi cả forceFlush lẫn debounce timer.
async function _flushSaveRecordNow(env, name, data) {
  const encrypted = await aesGcmEncryptToJson(env, data);
  await env.GAME_DATA.put('save:' + name, encrypted);
  _saveReadCache.set(name, { data, ts: Date.now() }); // cập nhật cache đọc luôn, tránh phải GET lại ngay sau khi PUT
}

// ---- Wrapper GHI xuống KV cho key 'save:'+name — LUÔN mã hoá trước khi put(). Dùng hàm này thay cho
//      env.GAME_DATA.put('save:'+name, JSON.stringify(data)) trực tiếp ở MỌI nơi trong file.
//      options.forceFlush=true -> ghi thẳng xuống KV ngay (bắt buộc dùng cho trade/sell/rename/export/admin).
//      options.ctx (ExecutionContext, có waitUntil) -> nếu có, đảm bảo lượt flush trì hoãn vẫn chạy xong
//      dù request hiện tại đã trả response, tránh mất dữ liệu khi Worker instance bị thu hồi giữa chừng. ----
async function writeSaveRecord(env, name, data, options = {}) {
  const { forceFlush = false, ctx = null } = options;

  if (forceFlush) {
    const pending = _saveWriteBuffer.get(name);
    if (pending && pending.timer) clearTimeout(pending.timer);
    _saveWriteBuffer.delete(name);
    await _flushSaveRecordNow(env, name, data);
    return;
  }

  // Cập nhật cache đọc ngay để các lệnh readSaveRecord() tiếp theo trong cùng cửa sổ thấy dữ liệu mới nhất.
  _saveReadCache.set(name, { data, ts: Date.now() });

  const existing = _saveWriteBuffer.get(name);
  if (existing) {
    existing.data = data; // gộp: chỉ giữ bản MỚI NHẤT, timer cũ vẫn chạy, không reset liên tục -> đảm bảo có trần thời gian flush
    return;
  }

  let resolveFlush;
  const pendingFlush = new Promise(res => { resolveFlush = res; });
  const entry = { data, timer: null, pendingFlush };
  entry.timer = setTimeout(async () => {
    const latest = _saveWriteBuffer.get(name);
    _saveWriteBuffer.delete(name);
    try {
      await _flushSaveRecordNow(env, name, latest ? latest.data : data);
    } catch (err) {
      // Không để lỗi flush nền làm crash Worker; dữ liệu vẫn còn trong _saveReadCache cho tới khi hết hạn.
    } finally {
      resolveFlush();
    }
  }, SAVE_BUFFER_DELAY_MS);
  _saveWriteBuffer.set(name, entry);

  if (ctx && typeof ctx.waitUntil === 'function') {
    ctx.waitUntil(pendingFlush); // đảm bảo Cloudflare không thu hồi instance trước khi lượt ghi trì hoãn hoàn tất
  }
}

// ---- Wrapper ĐỌC từ KV cho key 'save:'+name — tự nhận diện bản ghi đã mã hoá (envelope AES-GCM) hay
//      bản ghi kiểu cũ (JSON thường, chưa mã hoá) để tương thích ngược, giải mã nếu cần rồi trả về object
//      data gốc (hoặc null nếu key không tồn tại). Dùng hàm này thay cho
//      JSON.parse(await env.GAME_DATA.get('save:'+name)) trực tiếp ở MỌI nơi trong file.
//      Ưu tiên cache/buffer trong bộ nhớ trước khi hỏi KV -> giảm GET lặp lại khi nhiều hành động liên
//      tiếp cùng đọc-sửa-ghi trên cùng 1 account trong thời gian ngắn. ----
async function readSaveRecord(env, name) {
  const buffered = _saveWriteBuffer.get(name);
  if (buffered) return buffered.data; // có bản ghi đang chờ flush -> đây LUÔN là bản mới nhất, khỏi hỏi KV

  const cached = _saveReadCache.get(name);
  if (cached && (Date.now() - cached.ts) < SAVE_BUFFER_DELAY_MS) return cached.data;

  const raw = await env.GAME_DATA.get('save:' + name, { cacheTtl: 3600 }); // tăng cacheTtl edge (mặc định 60s) cho các lượt đọc xa nhau, ít thay đổi
  if (raw === null || raw === undefined) return null;
  let data;
  if (looksLikeAesEnvelope(raw)) {
    data = await aesGcmDecryptFromJson(env, raw);
  } else {
    // Bản ghi cũ chưa mã hoá — parse thẳng để tương thích ngược, không làm hỏng account đang chơi dở.
    data = JSON.parse(raw);
  }
  _saveReadCache.set(name, { data, ts: Date.now() });
  return data;
...
// ---- LỚP AES-GCM NGOÀI CÙNG NHẤT cho export code — dùng RIÊNG key ring EXPORT_WRAP_KEY_* (kid-based),
//      TÁCH BIỆT khỏi aesGcmEncryptToJson/aesGcmDecryptFromJson (module đó dành cho save data trong KV,
//      dùng SAVE_DATA_KEY, giữ nguyên không đổi để không phá logic save hiện có). Luôn mã hoá bằng key
//      CURRENT; giải mã tra theo kid nhúng trong envelope (không brute-force toàn bộ ring). ----
async function deriveWrapAesKey(secret) {
  const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(secret), 'HKDF', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: new TextEncoder().encode('export-wrap-aes-gcm-v5') },
    baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
  );
}
async function wrapAesEncryptToJson(env, plaintext) {
  const { kid, secret } = currentWrapKeyEntry(env);
  const key = await deriveWrapAesKey(secret);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode(plaintext));
  return JSON.stringify({ v: 1, kid, iv: bufToBase64Url(iv.buffer), cipher: bufToBase64Url(cipherBuf) });
}
async function wrapAesDecryptFromJson(env, jsonStr) {
  let envelope;
  try { envelope = JSON.parse(jsonStr); } catch (err) { throw new Error('Envelope không hợp lệ.'); }
  const { iv, cipher } = envelope || {};
  // kid mặc định 'wrap-legacy' cho envelope cũ (trước khi có kid) — dùng secret EXPORT_WRAP_KEY cũ để
  // không phá các mã export đã phát hành trước khi nâng cấp lên key ring.
  const kid = typeof envelope?.kid === 'string' && envelope.kid ? envelope.kid : 'wrap-legacy';
  if (!iv || typeof iv !== 'string' || !cipher || typeof cipher !== 'string') {
    throw new Error('Envelope thiếu iv/cipher.');
  }
  const secret = resolveWrapKeyByKid(env, kid);
  if (!secret) throw new Error('Khoá ứng với kid "' + kid + '" không xác định hoặc đã bị thu hồi.');
  const key = await deriveWrapAesKey(secret);
  const ivBytes = new Uint8Array(base64UrlToBuf(iv));
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ivBytes }, key, base64UrlToBuf(cipher));
  return new TextDecoder().decode(plainBuf);
}

/* ---- LỚP AES-GCM GIỮA (mới) — nằm NGAY SAU innerCode (base64url của ticket đã AES-GCM+HMAC-sign ở
   trong), TRƯỚC lớp HMAC-wrap + AES-GCM-wrap ngoài cùng. Dùng secret RIÊNG "EXPORT_MID_AES_KEY" — ĐỘC
   LẬP HOÀN TOÀN với ADMIN_KEY/EXPORT_SIGN_KEY/EXPORT_AES_KEY/EXPORT_WRAP_KEY_* (không dẫn xuất từ nhau,
   không dùng chung với bất kỳ lớp nào khác). Chuỗi lớp export hiện tại:
     data -> AES-GCM(EXPORT_AES_KEY) + HMAC-sign(EXPORT_SIGN_KEY) -> innerCode (base64url)
          -> AES-GCM(EXPORT_MID_AES_KEY)   [LỚP MỚI — thêm ở đây]
          -> HMAC-wrap(EXPORT_WRAP_KEY_CURRENT, kid) -> AES-GCM-wrap(EXPORT_WRAP_KEY_CURRENT, kid)
          -> base64url cuối cùng (export code người dùng copy/paste)
   Set bằng: wrangler secret put EXPORT_MID_AES_KEY
   Không có key ring/rotation cho lớp này (không bắt buộc theo yêu cầu) — nếu cần rotate sau này, áp dụng
   đúng pattern kid như EXPORT_WRAP_KEY_RING (thêm _CURRENT/_PREV_1/_PREV_2). ---------------------------- */
async function deriveMidAesKey(env) {
  if (!env.EXPORT_MID_AES_KEY) throw new Error('Server chưa cấu hình khoá lớp giữa (EXPORT_MID_AES_KEY).');
  const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(env.EXPORT_MID_AES_KEY), 'HKDF', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name: 'HKDF', hash: 'SHA-256', salt: new Uint8Array(0), info: new TextEncoder().encode('export-mid-aes-gcm-v1') },
    baseKey, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']
  );
}
async function midAesEncryptToJson(env, plaintext) {
  const key = await deriveMidAesKey(env);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode(plaintext));
  return JSON.stringify({ v: 1, iv: bufToBase64Url(iv.buffer), cipher: bufToBase64Url(cipherBuf) });
}
async function midAesDecryptFromJson(env, jsonStr) {
  let envelope;
  try { envelope = JSON.parse(jsonStr); } catch (err) { throw new Error('Envelope lớp giữa không hợp lệ (sai định dạng).'); }
  const { iv, cipher } = envelope || {};
  if (!iv || typeof iv !== 'string' || !cipher || typeof cipher !== 'string') {
    throw new Error('Envelope lớp giữa thiếu iv/cipher.');
  }
  const key = await deriveMidAesKey(env);
  const ivBytes = new Uint8Array(base64UrlToBuf(iv));
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ivBytes }, key, base64UrlToBuf(cipher));
...
// Nhận diện envelope lớp giữa cũ/không có (tương thích ngược cho mã export phát hành TRƯỚC khi thêm lớp
// này) — nếu chuỗi không parse được thành {v:1,iv,cipher} thì coi là mã kiểu cũ (chưa có lớp giữa),
// truyền thẳng KHÔNG giải mã lớp này, giữ nguyên hành vi cũ để không phá mã export đã phát hành trước đó.
function looksLikeMidAesEnvelope(jsonStr) {
  if (!jsonStr || typeof jsonStr !== 'string') return false;
  try {
    const obj = JSON.parse(jsonStr);
    return !!(obj && typeof obj === 'object' && obj.v === 1 && typeof obj.iv === 'string' && typeof obj.cipher === 'string');
...
// Tạo export code hoàn chỉnh cho 1 accountId, NHÚNG LUÔN save `data` hiện tại (đã MÃ HOÁ AES-GCM, IV
// ngẫu nhiên) + 1 nonce ngẫu nhiên mới (dùng để về sau đánh dấu chính XÁC mã này đã được import hay chưa),
// SAU ĐÓ bọc thêm 1 lớp HMAC-SHA256 + 1 lớp AES-GCM ngoài cùng, cả 2 đều dùng key CURRENT của key ring
// EXPORT_WRAP_KEY_* (kid được nhúng vào cả 2 lớp để phía đọc tra đúng key, không brute-force).
async function createExportCode(env, accountId, data) {
  const ts = Date.now();
  const nonce = genExportNonce();
  const { iv, cipher } = await encryptExportData(env, data);
  const signed = await signExportTicket(env, accountId, ts, nonce, iv, cipher);
  const ticket = { v: 4, accountId, ts, nonce, iv, cipher, signed };
  const innerCode = textToBase64Url(JSON.stringify(ticket));
  // LỚP MỚI (giữa): AES-GCM bằng secret RIÊNG EXPORT_MID_AES_KEY, ngay sau innerCode và TRƯỚC lớp HMAC-wrap
  // + AES-GCM-wrap ngoài cùng — xem chi tiết tại module midAesEncryptToJson/deriveMidAesKey phía trên.
  const midWrapped = await midAesEncryptToJson(env, innerCode);
  const hmacWrapped = await wrapOuterLayer(env, midWrapped);
  // LỚP THỨ TƯ (NGOÀI CÙNG THẬT SỰ): mã hoá AES-GCM chuỗi đã ký HMAC ở trên bằng key CURRENT của ring
  // EXPORT_WRAP_KEY_*. Kẻ tấn công không có secret này thì không đọc được NỘI DUNG mã export (kể cả chữ
  // ký HMAC/accountId/nonce) chứ không chỉ bị chặn xác thực.
  const aesEnvelopeJson = await wrapAesEncryptToJson(env, hmacWrapped);
  return textToBase64Url(aesEnvelopeJson);
}

// Giải mã + xác thực 1 export code do người dùng dán vào. Ném lỗi rõ ràng nếu sai định dạng/chữ ký giả
// mạo/dữ liệu bị sửa tay/mã hết hạn/mã đã dùng rồi. Trả về { accountId, ts, nonce, data } nếu hợp lệ —
// KHÔNG cần đọc KV save, vì data đã nằm sẵn trong chính mã (đã mã hoá + ký nên không thể bị sửa hay đọc
// trộm mà không bị phát hiện/ngăn chặn).
// LƯU Ý: hàm này CHỈ kiểm tra mã đã dùng CHƯA — việc ĐÁNH DẤU đã dùng do gọi nơi khác (markExportCodeUsed)
// thực hiện SAU KHI toàn bộ luồng import xử lý xong không lỗi, để tránh đánh dấu "đã dùng" một mã mà
// cuối cùng lại import thất bại vì lý do khác (vd tài khoản đã bị xoá).
async function verifyExportCode(env, code) {
  // Bóc lớp NGOÀI CÙNG THẬT SỰ trước tiên: giải mã AES-GCM (kid nhúng trong envelope tra ra đúng secret
  // trong key ring EXPORT_WRAP_KEY_*) — sai khoá/kid không xác định hoặc mã bị sửa dù 1 byte thì auth tag
  // chặn ngay tại đây, KHÔNG cần đụng tới HMAC/AES-GCM bên trong.
  let hmacWrapped;
  try {
    const aesEnvelopeJson = base64UrlToText(String(code || '').trim());
    hmacWrapped = await wrapAesDecryptFromJson(env, aesEnvelopeJson);
  } catch (err) {
    throw new Error('Mã import không hợp lệ (sai lớp bọc ngoài cùng — có thể mã bị sửa hoặc không phải do server này cấp).');
  }
  // Sau đó bóc lớp HMAC-SHA256 — kid nhúng trong lớp này (độc lập kid của lớp AES ngoài) tra ra đúng
  // secret trong key ring, không brute-force toàn bộ ring.
  const midWrapped = await unwrapOuterLayer(env, hmacWrapped);
  // Bóc LỚP GIỮA (mới, EXPORT_MID_AES_KEY) — nếu envelope không đúng định dạng {v:1,iv,cipher} thì coi
  // là mã export CŨ phát hành TRƯỚC khi thêm lớp này (chưa có lớp giữa) -> dùng thẳng midWrapped làm
  // innerCode, giữ tương thích ngược, không phá mã cũ.
  let innerCode;
  if (looksLikeMidAesEnvelope(midWrapped)) {
    try {
      innerCode = await midAesDecryptFromJson(env, midWrapped);
    } catch (err) {
      throw new Error('Mã import không hợp lệ (sai lớp giữa — có thể mã đã bị sửa hoặc không phải do server này cấp).');
    }
  } else {
    innerCode = midWrapped;
  }
  let ticket;
  try {
    ticket = JSON.parse(base64UrlToText(String(innerCode || '').trim()));
  } catch (err) {
    throw new Error('Mã import không hợp lệ (sai định dạng).');
  }
  const { accountId, ts, nonce, iv, cipher, signed } = ticket || {};
  // Mã kiểu cũ (trước v4) nhúng `data` dạng chữ rõ thay vì {iv,cipher} — không còn tương thích vì đã đổi
  // hoàn toàn sang mã hoá AES-GCM; báo lỗi rõ ràng để người dùng biết cần xuất lại mã mới.
  if (!accountId || typeof accountId !== 'string' || !ts || !nonce || !iv || !cipher || !signed) {
    throw new Error('Mã import thiếu dữ liệu bắt buộc (có thể là mã kiểu cũ trước khi có mã hoá AES-GCM — hãy Xuất lại mã mới).');
  }
  const expected = await signExportTicket(env, accountId, ts, nonce, iv, cipher);
  // So sánh độ dài trước rồi so từng ký tự — tránh nhánh rẽ sớm giúp giảm rủi ro timing attack
  // (dù rủi ro thực tế ở đây thấp vì chữ ký đã base64url hoá, không phải so sánh trực tiếp bí mật).
  if (expected.length !== signed.length) throw new Error('Mã import không hợp lệ (chữ ký sai) — có thể mã đã bị sửa hoặc không phải do server này cấp.');
  let diff = 0;
  for (let i = 0; i < expected.length; i++) diff |= expected.charCodeAt(i) ^ signed.charCodeAt(i);
  if (diff !== 0) throw new Error('Mã import không hợp lệ (chữ ký sai) — có thể mã đã bị sửa hoặc không phải do server này cấp.');
  // HẾT HẠN: quá EXPORT_CODE_MAX_AGE_MS kể từ lúc tạo -> từ chối, kể cả khi chữ ký đúng và chưa từng dùng.
  if (Date.now() - ts > EXPORT_CODE_MAX_AGE_MS) {
    throw new Error('Mã import đã hết hạn (mã chỉ có hiệu lực trong 24 giờ kể từ lúc Xuất) — hãy Xuất lại mã mới.');
  }
  // Chữ ký hợp lệ + chưa hết hạn -> kiểm tra xem nonce này đã từng được đánh dấu "đã dùng" chưa.
  const usedRaw = await env.GAME_DATA.get('used-export:' + nonce);
  if (usedRaw) throw new Error('Mã import này đã được sử dụng trước đó — mỗi mã export chỉ dùng được 1 lần. Hãy Xuất lại mã mới nếu cần chuyển máy lần nữa.');
  // Chữ ký + hạn dùng đều ổn -> giờ mới giải mã AES-GCM (nếu cipher/iv bị sửa tay bằng cách nào đó vẫn
  // qua được so khớp HMAC ở trên — về lý thuyết gần như không thể — auth tag của AES-GCM sẽ chặn ở đây).
  let data;
  try {
    data = await decryptExportData(env, iv, cipher);
  } catch (err) {
    throw new Error('Mã import không hợp lệ (giải mã thất bại) — dữ liệu có thể đã bị sửa.');
  }
  return { accountId, ts, nonce, data: data === undefined ? null : data };
}

// Đánh dấu 1 nonce là ĐÃ DÙNG — gọi SAU KHI toàn bộ luồng /account/import xử lý xong KHÔNG LỖI (tài
// khoản còn tồn tại, không bị ban...), để tránh trường hợp mã bị "đốt" (đánh dấu đã dùng) dù cuối cùng
// import lại thất bại vì lý do khác không liên quan tới bản thân mã (khi đó người dùng nên thử lại được
// với chính mã cũ, không cần Xuất mã mới). TTL 1 năm — đủ dài để coi như vĩnh viễn trong vòng đời game,
// nhưng vẫn tự dọn rác KV về lâu dài thay vì giữ record 'used-export:' vô thời hạn.
async function markExportCodeUsed(env, nonce) {
  await env.GAME_DATA.put('used-export:' + nonce, String(Date.now()), { expirationTtl: 31536000 });
}
...
export class GlobalState {
  constructor(state, env) {
    ...
    // Rate limit IN-MEMORY cho các thao tác trade (gift/offer) — theo tên account gửi, không theo IP
    // (nhiều account thật có thể chung 1 mạng/NAT). DO này là singleton xử lý tuần tự nên không cần
    // đồng bộ KV chéo isolate như checkRateLimit() — giảm hẳn GET/PUT KV cho rate limit trade.
    this.tradeActionRate = new Map(); // normName -> { count, windowStart }
  }
  // true nếu actorName được phép thực hiện 1 thao tác trade (gift/offer create/respond...), tự tăng đếm.
  checkTradeActionRate(actorName) {
    const TRADE_MAX_PER_WINDOW = 10; // tối đa 10 thao tác trade
    const TRADE_WINDOW_MS = 10000;   // mỗi 10 giây
    const now = Date.now();
    let entry = this.tradeActionRate.get(actorName);
    if (!entry || now - entry.windowStart >= TRADE_WINDOW_MS) {
      entry = { count: 0, windowStart: now };
      this.tradeActionRate.set(actorName, entry);
      if (this.tradeActionRate.size > 2000) {
        for (const [k, v] of this.tradeActionRate) { if (now - v.windowStart >= TRADE_WINDOW_MS) this.tradeActionRate.delete(k); }
      }
    }
    if (entry.count >= TRADE_MAX_PER_WINDOW) return false;
    entry.count += 1;
    return true;
  }
  async ensureLoaded() {
  ...
  async fetch(request) {
    ...
    // ---- Serialize toàn bộ đọc-tính-ghi của MỘT account save (chỉ gọi qua getAccountLockStub, tức 1
    // instance GlobalState RIÊNG theo tên account — KHÔNG phải instance 'singleton' quản lý events/trade
    // ở trên). Vì instance DO xử lý request tuần tự, việc đọc snapshot MỚI NHẤT + ghi xảy ra ở ĐÚNG 1
    // instance này chặn hẳn lost-update giữa 2 request /save gần như đồng thời của cùng account — không
    // chỉ phát hiện (như optimistic check ở worker chính) mà loại bỏ hẳn race window. Endpoint nội bộ,
    // KHÔNG lộ ra ngoài qua router chính — chỉ worker gọi qua stub.fetch('https://internal/...'). ----
    if (path.endsWith('/internal/save-commit') && request.method === 'POST') {
      const body = await request.json();
      const { name, metaToWrite, nameRecordToWrite } = body;
      // Đọc state MỚI NHẤT NGAY TRONG instance tuần tự này (không phải ở worker chính) — vì DO xử lý
      // request tuần tự, không có request /save nào khác của CÙNG account xen được giữa đọc và ghi ở đây.
      const current = await readMergedPlayerRecord(this.env, name);
      if (body.expectedSavedAt !== undefined && body.expectedSavedAt !== null) {
        const actualSavedAt = current.saveData ? current.saveData._savedAt : null;
        if (actualSavedAt !== body.expectedSavedAt) {
          return json({ error: 'Dữ liệu đã được lưu bởi phiên khác sau thời điểm bạn tải — tải lại trước khi lưu tiếp.', code: 'SAVE_CONFLICT', latestSavedAt: actualSavedAt }, 409);
        }
      }
      const rawData = (body.data && typeof body.data === 'object') ? body.data : {};
      rawData._displayName = body.displayName || '';
      rawData._lbAnonymous = !!rawData.lbAnonymous;
      rawData._savedAt = Date.now();
      await writeMergedPlayerRecord(this.env, name, {
        saveData: rawData,
        metaData: metaToWrite,
        nameRecord: nameRecordToWrite,
      });
      return json({ ok: true, savedAt: rawData._savedAt });
    }
    ...
    if (path.endsWith('/trade/gift') && request.method === 'POST') {
      ...
      if (!this.checkTradeActionRate(fromName)) return json({ error: 'Thao tác trade quá nhanh, chững lại 1 chút.' }, 429);
      ...
      if (fromName === toName) {
        try {
          const data = await readSaveRecord(this.env, fromName);
          if (!data) throw new Error('Không tìm thấy dữ liệu người chơi.');
  ...
  async applyItemTransfer(fromName, toName, items) {
    const fromData = await readSaveRecord(this.env, fromName);
    let toData = await readSaveRecord(this.env, toName);
    if (!fromData) throw new Error('Không tìm thấy dữ liệu người gửi.');
    // FIX: trước đây nếu người nhận (vd. tài khoản admin nhận Gold khi mua Dark Blade trong Shop) chưa từng
    // online/save lần nào, toData sẽ là null -> ném lỗi -> KHÔNG MUA ĐƯỢC DARK BLADE dù đủ Gold. Tự tạo 1 save
    // rỗng hợp lệ cho người nhận ngay tại đây thay vì chặn hẳn giao dịch.
    if (!toData) {
      toData = { gold: 0, fruits: [], weapons: [], guns: [] };
      await writeSaveRecord(this.env, toName, toData, { forceFlush: true });
    }
    removeItemsChecked(fromData, items); // ném lỗi nếu fromData không thật sự có đủ item claimed
    addItems(toData, items);
    // Trade là nghiệp vụ toàn vẹn cao — ghi thẳng KV ngay, không gom buffer, tránh mất giao dịch item.
    await writeSaveRecord(this.env, fromName, fromData, { forceFlush: true });
    await writeSaveRecord(this.env, toName, toData, { forceFlush: true });
  ...
  async settleTrade(offer) {
    const fromData = await readSaveRecord(this.env, offer.fromName);
    const toData = await readSaveRecord(this.env, offer.toName);
    if (!fromData || !toData) throw new Error('Một trong hai bên không còn dữ liệu trên server.');
    ...
    // Trade 2 chiều — ghi thẳng KV ngay, không gom buffer.
    await writeSaveRecord(this.env, offer.fromName, fromData, { forceFlush: true });
    await writeSaveRecord(this.env, offer.toName, toData, { forceFlush: true });
...
/* ============================================================
   ...
   ============================================================ */
// Cấu hình Season Pass MẶC ĐỊNH khi chưa admin nào set — 10 tier.
// XP yêu cầu: tier 1 cần 1.000.000 XP, mỗi tier sau x1,5 lần tier trước (cộng dồn theo yêu cầu mới).
// Free track: Gold x100 so với gốc cũ. Premium track (yêu cầu đã mua premium = admin cấp qua
// /season/grant-premium hoặc mua bằng Gold — xem endpoint /season/buy-premium): Gold x500 so với gốc cũ + vé Gacha.
function defaultSeason() {
  const tiers = [];
  let xpReq = 1000000; // tier 1: tối thiểu 1.000.000 XP
  for (let t = 1; t <= 10; t++) {
    tiers.push({
      tier: t,
      xpReq: Math.round(xpReq),
      freeReward: { gold: 2000 * t * 100 }, // x100 so với gốc (2000 -> 200000 mỗi tier)
      premiumReward: { gold: 5000 * t * 500, gachaTickets: t }, // x500 so với gốc (5000 -> 2.500.000 mỗi tier)
    });
    xpReq *= 1.5; // mỗi bậc kế tiếp cần x1,5 XP so với bậc trước
  }
  return {
    seasonId: 'season1',
    name: 'Mùa 1',
    endsAt: Date.now() + 30 * 24 * 60 * 60 * 1000, // mặc định 30 ngày kể từ lúc khởi tạo lần đầu
    premiumPriceGold: 50000000, // giá Premium Season Pass mới: 50.000.000
    tiers,
  };
}
...
export class GuildState {
  constructor(state, env) {
    ...
    // Rate limit RIÊNG cho /arena/hit — hoàn toàn IN-MEMORY (không hỏi KV), vì đây là endpoint dồn dập
    // nhất trong game (mỗi đòn đánh PvP gọi 1 lần) và DO này đã là instance singleton xử lý tuần tự, nên
    // không cần đồng bộ chéo isolate như checkRateLimit() dùng cho các endpoint qua Worker thường. Giới
    // hạn: tối đa ARENA_HIT_MAX_PER_WINDOW đòn / ARENA_HIT_WINDOW_MS mỗi người chơi — đủ nhanh cho combat
    // bình thường nhưng chặn được script auto-click/spam request giả đòn đánh.
    this.arenaHitRate = new Map(); // normName -> { count, windowStart }
  }
  // true nếu attacker được phép đánh (chưa vượt rate limit), tự tăng bộ đếm nếu cho phép.
  checkArenaHitRate(attackerName) {
    const ARENA_HIT_MAX_PER_WINDOW = 15; // tối đa 15 đòn
    const ARENA_HIT_WINDOW_MS = 5000;    // mỗi 5 giây — ~3 đòn/giây, đủ cho combat bình thường
    const now = Date.now();
    let entry = this.arenaHitRate.get(attackerName);
    if (!entry || now - entry.windowStart >= ARENA_HIT_WINDOW_MS) {
      entry = { count: 0, windowStart: now };
      this.arenaHitRate.set(attackerName, entry);
      // Dọn bớt entry cũ để Map không phình vô hạn theo thời gian server chạy.
      if (this.arenaHitRate.size > 2000) {
        for (const [k, v] of this.arenaHitRate) { if (now - v.windowStart >= ARENA_HIT_WINDOW_MS) this.arenaHitRate.delete(k); }
      }
    }
    if (entry.count >= ARENA_HIT_MAX_PER_WINDOW) return false;
    entry.count += 1;
    return true;
  }
  async ensureLoaded() {
  ...
  async finishArenaMatch(match, winner, loser) {
    ...
    if (winnerData) {
      ...
      await writeSaveRecord(this.env, winner, winnerData);
    }
    if (loserData) {
      loserData.gold = (loserData.gold || 0) + LOSE_GOLD;
      await writeSaveRecord(this.env, loser, loserData);
    }
  ...
  async fetch(request) {
    ...
    if (path.endsWith('/pvp/report') && request.method === 'POST') {
      ...
      winnerData.gold = (winnerData.gold || 0) + WIN_GOLD;
      loserData.gold = (loserData.gold || 0) + LOSE_GOLD;
      await writeSaveRecord(this.env, winnerName, winnerData);
      await writeSaveRecord(this.env, loserName, loserData);
    ...
    if (path.endsWith('/season/claim') && request.method === 'POST') {
      ...
      await writeSaveRecord(this.env, name, data, { forceFlush: true }); // nhận thưởng 1 lần — ghi thẳng, tránh nhận trùng nếu buffer mất
    ...
    if (path.endsWith('/season/buy-premium') && request.method === 'POST') {
      ...
      data.gold -= price;
      await writeSaveRecord(this.env, name, data, { forceFlush: true }); // mua Premium Pass — trừ Gold ngay, ghi thẳng
    ...
    if (path.endsWith('/arena/hit') && request.method === 'POST') {
      ...
      if (!this.checkArenaHitRate(attackerName)) return json({ error: 'Đánh quá nhanh, chững lại 1 chút.' }, 429);
...
// Stub Durable Object GlobalState nhưng KHÁC instance singleton ở trên — 1 instance RIÊNG cho mỗi tên
// account (idFromName(name)), dùng LÀM LÀN TUẦN TỰ (serialize) cho save/commit của CHÍNH account đó.
// Vì mỗi DO instance xử lý request tuần tự (Cloudflare đảm bảo), 2 request /save gần như đồng thời của
// CÙNG 1 account sẽ luôn đi qua đúng 1 instance và được xử lý lần lượt — chặn hẳn lost-update thay vì chỉ
// phát hiện bằng optimistic check. Account khác nhau -> instance khác nhau -> KHÔNG bị xếp hàng chung,
// nên không tạo cổ chai hiệu năng toàn server (không phải self-DOS, khác hẳn nếu dùng chung 'singleton').
function getAccountLockStub(env, name) {
  const id = env.GLOBAL.idFromName('save-lock:' + name);
  return env.GLOBAL.get(id);
}
...
function checkAdminKey(env, key) {
  return !!env.ADMIN_KEY && typeof key === 'string' && timingSafeEqualStr(key, env.ADMIN_KEY);
}
// LỚP 1 (mục 4): ưu tiên đọc Admin Key từ header `Authorization: Bearer <key>` (không rơi vào URL/query
// log của proxy/CDN); vẫn CHẤP NHẬN body.key / ?key= để tương thích ngược với client cũ chưa cập nhật.
function extractAdminKey(request, url, body) {
...
// LỚP 2 (mục 3, ĐÃ SỬA đúng vai trò): "ADMIN_PROTECT_SECRET" KHÔNG phải 1 mật khẩu thứ hai so trực tiếp —
// nó là secret dùng để KÝ (HMAC-SHA256) 1 "phiên admin" (session token) CẤP RA sau khi Admin Key đã được
// xác minh đúng, để BẢO VỆ chính Admin Key khỏi phải gửi lặp lại trên mạng ở mọi lệnh admin sau đó.
// Luồng:
//   1) Client gửi Admin Key thật (qua Authorization: Bearer) tới /admin/verify.
//   2) Nếu đúng Admin Key: server KÝ 1 session { ts, nonce, sig=HMAC(ADMIN_PROTECT_SECRET, ts+nonce) },
//      trả về cho client dưới dạng 1 chuỗi base64url. Session có hạn dùng ADMIN_SESSION_MAX_AGE_MS.
//   3) Các lệnh admin SAU ĐÓ (/admin/command, /admin/players/all, /admin/log, /admin/event/set) chỉ cần
//      gửi session token này (header X-Admin-Session) — KHÔNG cần gửi lại Admin Key mỗi lần nữa.
// Nếu server CHƯA cấu hình ADMIN_PROTECT_SECRET: fail-open có chủ đích — quay lại xác thực bằng thẳng
// Admin Key ở mọi request như trước (không khoá cứng admin hiện tại trước khi kịp `wrangler secret put`).
// LUÔN set ADMIN_PROTECT_SECRET trên môi trường production để bật lớp bảo vệ này.
const ADMIN_SESSION_MAX_AGE_MS = 12 * 60 * 60 * 1000; // 12 giờ — đủ dài cho 1 phiên làm việc admin, không vĩnh viễn.
async function signAdminSession(env, ts, nonce) {
  const keyMat = await crypto.subtle.importKey(
    'raw', new TextEncoder().encode(env.ADMIN_PROTECT_SECRET),
    { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const sig = await crypto.subtle.sign('HMAC', keyMat, new TextEncoder().encode('admin-session:' + ts + ':' + nonce));
  return bufToBase64Url(sig);
}
// Cấp 1 session token mới — gọi SAU KHI đã xác minh đúng Admin Key thật (checkAdminKey trả true).
async function issueAdminSession(env) {
  const ts = Date.now();
  const nonce = genExportNonce(); // tái dùng hàm sinh nonce 16 byte ngẫu nhiên đã có sẵn cho export
  const sig = await signAdminSession(env, ts, nonce);
  return textToBase64Url(JSON.stringify({ ts, nonce, sig }));
}
// Xác minh 1 session token: giải mã, KÝ LẠI đúng {ts,nonce} rồi so constant-time với sig đi kèm, và kiểm
// tra chưa hết hạn. KHÔNG cần tra KV (session không dùng 1 lần như export — admin có thể gọi nhiều lệnh
// trong 1 phiên) — tính stateless, chỉ dựa vào chữ ký + thời hạn.
async function verifyAdminSession(env, token) {
  if (!env.ADMIN_PROTECT_SECRET || typeof token !== 'string' || !token) return false;
  let parsed;
  try { parsed = JSON.parse(base64UrlToText(token)); } catch (err) { return false; }
  const { ts, nonce, sig } = parsed || {};
  if (typeof ts !== 'number' || typeof nonce !== 'string' || typeof sig !== 'string') return false;
  if (Date.now() - ts > ADMIN_SESSION_MAX_AGE_MS) return false;
  let expected;
  try { expected = await signAdminSession(env, ts, nonce); } catch (err) { return false; }
  if (expected.length !== sig.length) return false;
  let diff = 0;
  for (let i = 0; i < expected.length; i++) diff |= expected.charCodeAt(i) ^ sig.charCodeAt(i);
  return diff === 0;
}
function extractAdminSession(request, url, body) {
  const header = request.headers.get('X-Admin-Session');
  if (typeof header === 'string' && header) return header;
  if (body && typeof body.adminSession === 'string') return body.adminSession;
  return url.searchParams.get('adminSession') || '';
}
// Gộp xác thực admin thật thành 1 lần gọi — dùng cho MỌI endpoint chỉ-admin-thật (command/players/log/event).
// - Nếu ADMIN_PROTECT_SECRET đã cấu hình: BẮT BUỘC có session token hợp lệ (Admin Key gốc không còn được
//   chấp nhận trực tiếp ở các endpoint này nữa — phải đổi qua session tại /admin/verify trước).
// - Nếu CHƯA cấu hình ADMIN_PROTECT_SECRET: fail-open, quay lại so Admin Key trực tiếp như trước.
function isRealAdminRequest(env, request, url, body) {
  if (env.ADMIN_PROTECT_SECRET) {
    return verifyAdminSession(env, extractAdminSession(request, url, body));
  }
  return checkAdminKey(env, extractAdminKey(request, url, body));
}
...
async function loadSave(env, name, label) {
  const data = await readSaveRecord(env, name);
  if (!data) throw new Error('Không tìm thấy người chơi "' + label + '".');
  return data;
}
...
async function runAdminCommand(env, cmd, args) {
  ...
  switch (cmd) {
    ...
    case 'setLevel': {
      ...
      await writeSaveRecord(env, name, data, { forceFlush: true }); // lệnh admin — ghi thẳng ngay, admin cần thấy hiệu lực tức thì
    ...
    case 'ban': {
      ...
      await cachedKvPut(env, 'banned:' + name, JSON.stringify({ bannedAt: Date.now() }));
    ...
    case 'banHwid': {
      ...
      if (metaRaw) {
        ...
        if (ip && ip !== 'unknown') await cachedKvPut(env, 'banned-ip:' + ip, JSON.stringify({ bannedAt: Date.now(), fromName: name }));
        if (deviceId) await cachedKvPut(env, 'banned-device:' + deviceId, JSON.stringify({ bannedAt: Date.now(), fromName: name }));
      }
    ...
    case 'unban': {
      if (!name) throw new Error('Thiếu tên người chơi.');
      await cachedKvDelete(env, 'banned:' + name);
      const metaRaw = await cachedKvGet(env, 'meta:' + name, KV_CACHE_TTL_LONG_MS);
      if (metaRaw) {
        const meta = JSON.parse(metaRaw);
        if (meta.ip && meta.ip !== 'unknown') await cachedKvDelete(env, 'banned-ip:' + meta.ip);
        if (meta.deviceId) await cachedKvDelete(env, 'banned-device:' + meta.deviceId);
      }
    ...
    case 'checkBan': {
      ...
      const [nameBanned, ipBanned, deviceBanned] = await Promise.all([
        cachedKvGet(env, 'banned:' + name, KV_CACHE_TTL_SHORT_MS),
        meta && meta.ip && meta.ip !== 'unknown' ? cachedKvGet(env, 'banned-ip:' + meta.ip, KV_CACHE_TTL_SHORT_MS) : Promise.resolve(null),
        meta && meta.deviceId ? cachedKvGet(env, 'banned-device:' + meta.deviceId, KV_CACHE_TTL_SHORT_MS) : Promise.resolve(null),
      ]);
      return {
        name,
    ...
    case 'unbanIp': {
      ...
      await cachedKvDelete(env, 'banned-ip:' + ip);
      return { ip, banned: false };
    }
    case 'unbanDevice': {
      const deviceId = (args.deviceId || '').trim();
      if (!deviceId) throw new Error('Thiếu deviceId.');
      await cachedKvDelete(env, 'banned-device:' + deviceId);
    ...
    case 'kick': {
      ...
      await cachedKvPut(env, 'kicked:' + name, JSON.stringify({ kickedAt: Date.now() }), { expirationTtl: 300 });
    ...
    case 'giveAll': {
      ...
      do {
        ...
        for (const k of page.keys) {
          ...
          if (!raw) continue;
          let data;
          try { data = looksLikeAesEnvelope(raw) ? await aesGcmDecryptFromJson(env, raw) : JSON.parse(raw); } catch (e) { continue; }
          ...
          await env.GAME_DATA.put(k.name, await aesGcmEncryptToJson(env, data));
          count++;
        }
        cursor = page.cursor;
...
export default {
  // Wrapper mỏng: gọi handleFetch() để xử lý logic thật, rồi hậu xử lý CORS trên response TRẢ VỀ TRƯỚC
  // KHI gửi cho trình duyệt (xem applyCors() ở đầu file) — không cần sửa hàng trăm điểm gọi json() rải
  // rác bên trong handleFetch, giảm rủi ro sửa nhầm logic nghiệp vụ khi chỉ đang hardening CORS.
...
async function handleFetch(request, env) {
    ...
    if (request.method === 'OPTIONS') {
      return applyCors(request, new Response(null, { headers: CORS_HEADERS }));
    }
    ...
    try {
      ...
      if (path === '/my-status' && request.method === 'GET') {
        ...
        const [bannedRaw, kickedRaw, ipBannedRaw, deviceBannedRaw] = await Promise.all([
          cachedKvGet(env, 'banned:' + name, KV_CACHE_TTL_SHORT_MS),
          cachedKvGet(env, 'kicked:' + name, KV_CACHE_TTL_SHORT_MS),
          ip && ip !== 'unknown' ? cachedKvGet(env, 'banned-ip:' + ip, KV_CACHE_TTL_SHORT_MS) : Promise.resolve(null),
          deviceId ? cachedKvGet(env, 'banned-device:' + deviceId, KV_CACHE_TTL_SHORT_MS) : Promise.resolve(null),
        ]);
      ...
      if (path === '/register-name' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'register-name', getClientIp(request), 10, 300))) return rateLimitedResponse(300);
        const body = await request.json();
        ...
        const ip = getClientIp(request);
        await writeMergedPlayerRecord(env, name, {
          nameRecord: { registeredAt: Date.now(), token },
          metaData: { ip, deviceId: body.deviceId || null, updatedAt: Date.now() },
        });
      ...
      if (path === '/account/claim-token' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'claim-token', getClientIp(request), 10, 300))) return rateLimitedResponse(300);
        const body = await request.json();
        const name = normName(body.name);
        if (!name) return json({ error: 'Thiếu tên tài khoản.' }, 400);
        const banReason = await checkBanned(env, name, request, body.deviceId);
        if (banReason) return json({ error: banReason }, 403);
        const { metaData: existingMeta, nameRecord } = await readMergedPlayerRecord(env, name);
        if (!nameRecord) return json({ error: 'Tài khoản "' + name + '" chưa được đăng ký.' }, 404);
        if (!nameRecord.token) nameRecord.token = genToken();
        const ip = getClientIp(request);
        const newDeviceId = body.deviceId || (existingMeta ? existingMeta.deviceId : null) || null;
        await writeMergedPlayerRecord(env, name, {
          nameRecord,
          metaData: { ip, deviceId: newDeviceId, updatedAt: Date.now() },
        });
        return json({ ok: true, token: nameRecord.token });
      }
      ...
      // ---- POST /account/rename  body: { name, newName, token, deviceId } ----  (KHÔNG cần admin key —
      // người chơi tự đổi TÊN THẬT của TÀI KHOẢN CỦA CHÍNH MÌNH trên server, giữ nguyên token/save/tiến
      // trình — "Đổi tên không mất account". Trước đây renameSession() ở client CHỈ đổi tên hiển thị cục
      // bộ (nhãn của Phiên trên máy), KHÔNG hề đụng tới tên thật đăng ký trên server -> gây lệch giữa tên
      // hiển thị và accountId thật. Nay endpoint này đổi luôn CẢ 2 cùng lúc: chuyển toàn bộ record
      // ('name:'+cũ, 'save:'+cũ, 'meta:'+cũ) sang key mới rồi xoá key cũ, atomically theo thứ tự an toàn
      // (ghi mới xong mới xoá cũ, không xoá cũ trước phòng lỗi giữa chừng mất trắng tài khoản). ----
      if (path === '/account/rename' && request.method === 'POST') {
        const body = await request.json();
        const name = normName(body.name);
        const newName = normName(body.newName);
        if (!name) return json({ error: 'Thiếu tên tài khoản hiện tại.' }, 400);
        if (!newName) return json({ error: 'Tên mới không hợp lệ.' }, 400);
        if (name === newName) return json({ error: 'Tên mới trùng với tên hiện tại.' }, 400);
        if (await isProtectedAdmin(env, name)) return json({ error: 'Không thể đổi tên tài khoản admin.' }, 403);
        try { await verifyAccountToken(env, name, body.token, body.deviceId, true); }
        catch (err) { return json({ error: err.message }, 403); }
        const banReason = await checkBanned(env, newName, request, body.deviceId);
        if (banReason) return json({ error: banReason }, 403);
        const existingNew = await env.GAME_DATA.get('name:' + newName);
        if (existingNew) return json({ error: 'Tên này đã có người dùng.' }, 409);
        const [oldRecordRaw, oldSaveRaw, oldMetaRaw] = await Promise.all([
          env.GAME_DATA.get('name:' + name),
          env.GAME_DATA.get('save:' + name),
          cachedKvGet(env, 'meta:' + name, KV_CACHE_TTL_LONG_MS),
        ]);
        if (!oldRecordRaw) return json({ error: 'Tài khoản "' + name + '" không còn tồn tại trên server.' }, 404);
        // Ghi bản ghi MỚI trước (name/save/meta) — nếu bước này lỗi giữa chừng thì tài khoản CŨ vẫn còn
        // nguyên vẹn, không mất account. Chỉ sau khi ghi mới THÀNH CÔNG mới xoá bản CŨ.
        // GHI GỘP: meta của tên MỚI được tính sẵn trên bộ nhớ (ip/deviceId hiện tại) và đưa vào CHUNG 1
        // Promise.all với name/save — thay vì trước đây recordPlayerMeta() gọi riêng SAU KHI đã xoá key cũ
        // (thêm 1 round-trip KV độc lập cho cùng 1 request).
        const ip = getClientIp(request);
        let oldDeviceId = null;
        if (oldMetaRaw) { try { oldDeviceId = JSON.parse(oldMetaRaw).deviceId || null; } catch (err) { /* ignore */ } }
        const newDeviceId = body.deviceId || oldDeviceId || null;
        const writes = [
          env.GAME_DATA.put('name:' + newName, oldRecordRaw),
          cachedKvPut(env, 'meta:' + newName, JSON.stringify({ ip, deviceId: newDeviceId, updatedAt: Date.now() })),
        ];
        if (oldSaveRaw) writes.push(env.GAME_DATA.put('save:' + newName, oldSaveRaw));
        await Promise.all(writes);
        await Promise.all([
          env.GAME_DATA.delete('name:' + name),
          env.GAME_DATA.delete('save:' + name),
          cachedKvDelete(env, 'meta:' + name),
          env.GAME_DATA.delete('lb:' + name),
        ]);
        let record; try { record = JSON.parse(oldRecordRaw); } catch (err) { record = {}; }
        return json({ ok: true, name: newName, token: record.token || body.token || null });
      }

      // ---- POST /account/export  body: { name, token, deviceId } ----
      // Cấp 1 MÃ EXPORT cho tài khoản CỦA CHÍNH NGƯỜI GỌI — bắt buộc xác thực quyền sở hữu y hệt /save
      // (token khớp, hoặc deviceId khớp máy đã đăng ký) để chặn việc lấy trộm mã export tài khoản người khác
      // chỉ bằng cách biết tên (tên là công khai, hiển thị trên bảng xếp hạng/chat).
      // allowClaim=true: tài khoản cũ (tạo trước khi có token) cũng tự nâng cấp được như /save.
      // ---- POST /account/export  body: { name, token, deviceId, data } ----
      // Cấp 1 MÃ EXPORT cho tài khoản CỦA CHÍNH NGƯỜI GỌI — bắt buộc xác thực quyền sở hữu y hệt /save
      // (token khớp, hoặc deviceId khớp máy đã đăng ký) để chặn việc lấy trộm mã export tài khoản người khác
      // chỉ bằng cách biết tên (tên là công khai, hiển thị trên bảng xếp hạng/chat).
      // allowClaim=true: tài khoản cũ (tạo trước khi có token) cũng tự nâng cấp được như /save.
      // `data` (client gửi lên, thường là bản save mới nhất vừa đồng bộ) được NHÚNG LUÔN vào mã đã ký —
      // nên /account/import sau này verify được chữ ký + đọc luôn data từ trong mã, không bắt buộc phải
      // còn 'save:'+name trên KV nữa (dù vẫn ưu tiên dùng bản KV nếu có, xem /account/import).
      if (path === '/account/export' && request.method === 'POST') {
        const body = await request.json();
        const name = normName(body.name);
        if (!name) return json({ error: 'Thiếu tên tài khoản.' }, 400);
        try { await verifyAccountToken(env, name, body.token, body.deviceId, true); }
        catch (err) { return json({ error: err.message }, 403); }
        // Nếu client không gửi kèm data (vd gọi API trực tiếp không qua UI), lấy tạm bản KV hiện có —
        // để mã export luôn có data nhúng sẵn, không bao giờ trả về mã "rỗng".
        let dataToEmbed = body.data;
        if (dataToEmbed === undefined || dataToEmbed === null) {
          dataToEmbed = await readSaveRecord(env, name);
        }
        let code;
        try { code = await createExportCode(env, name, dataToEmbed); }
        catch (err) { return json({ error: err.message }, 500); }
        return json({ ok: true, code });
      }

      // ---- POST /account/import  body: { code } ----
      // Xác minh mã export (chữ ký EXPORT_SIGN_KEY, bao trùm cả accountId+ts+data nên không sửa tay được) ->
      // nếu hợp lệ, trả về accountId + token thật của tài khoản đó (đọc từ record 'name:'+accountId) để
      // client lưu lại làm token cục bộ trên máy mới. SAVE DATA: ưu tiên bản đang lưu trên KV server
      // ('save:'+name) NẾU CÓ (vì có thể mới hơn data nhúng trong mã — mã có thể được tạo từ lâu); chỉ
      // khi server KHÔNG CÒN giữ bản nào (đã bị xoá/dọn) mới dùng data nhúng sẵn trong chính mã export
      // làm phương án dự phòng — đây là điểm khác biệt chính so với bản trước (không nhúng data), giúp
      // Import không còn phụ thuộc hoàn toàn vào server còn giữ KV hay không.
      if (path === '/account/import' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'account-import', getClientIp(request), 6, 300))) return rateLimitedResponse(300);
        const body = await request.json();
        let ticket;
        try { ticket = await verifyExportCode(env, body.code); }
        catch (err) { return json({ error: err.message }, 400); }
        const accountId = ticket.accountId;
        const banReason = await checkBanned(env, accountId, request, body.deviceId);
        if (banReason) return json({ error: banReason }, 403);
        const { saveData: serverData, metaData: existingMeta, nameRecord } = await readMergedPlayerRecord(env, accountId);
        if (!nameRecord) return json({ error: 'Tài khoản "' + accountId + '" không còn tồn tại trên server (có thể đã bị xoá).' }, 404);
        // Idempotent giống /account/claim-token: nếu record cũ chưa có token thì cấp mới, còn có rồi thì trả luôn.
        if (!nameRecord.token) nameRecord.token = genToken();
        const ip = getClientIp(request);
        const newDeviceId = body.deviceId || (existingMeta ? existingMeta.deviceId : null) || null;
        await writeMergedPlayerRecord(env, accountId, {
          nameRecord,
          metaData: { ip, deviceId: newDeviceId, updatedAt: Date.now() },
        });
        // Mọi bước xác thực + đọc dữ liệu đã xong KHÔNG LỖI -> giờ mới "đốt" mã (đánh dấu nonce đã dùng),
        // để mã KHÔNG bị vô hiệu hoá nếu import thất bại vì lý do khác (tài khoản bị xoá, bị ban...) —
        // người dùng vẫn còn dùng lại được đúng mã đó sau khi lý do thất bại kia được xử lý.
        await markExportCodeUsed(env, ticket.nonce);
        return json({
          ok: true,
          name: accountId,
          token: nameRecord.token,
          data: serverData !== null ? serverData : ticket.data,
          usedEmbeddedData: serverData === null, // cho client biết đã dùng data nhúng trong mã (server không còn KV) hay bản mới trên server
        });
      }
      ...
      // HỢP NHẤT ĐỌC/GHI: đọc gộp cả 3 key (save:/meta:/name:) 1 lần bằng readMergedPlayerRecord, xử lý
      // xác thực + validate + cập nhật meta trên bộ nhớ, RỒI mới ghi gộp cả 3 lại bằng writeMergedPlayerRecord
      // trong đúng 1 Promise.all — thay vì trước đây verifyAccountToken/writeSaveRecord/recordPlayerMeta
      // mỗi hàm tự get/put độc lập (nhiều round-trip KV rời rạc hơn cho cùng 1 request).
      if (path === '/save' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'save', getClientIp(request), 60, 60))) return rateLimitedResponse(60);
        const body = await request.json();
        const name = normName(body.name);
        ...
        const { metaData: existingMeta, nameRecord: existingNameRecord } = await readMergedPlayerRecord(env, name);
        if (!existingNameRecord) return json({ error: 'Tài khoản "' + name + '" chưa được đăng ký.' }, 403);
        ...
        let nameRecordToWrite; // undefined = không cần ghi lại name:
        const token = body.token;
        if (existingNameRecord.token) {
          if (!token || !timingSafeEqualStr(token, existingNameRecord.token)) {
            return json({ error: 'Xác thực thất bại — đây không phải tài khoản của bạn.' }, 403);
          }
        } else if (token) {
          // Tài khoản cũ chưa có token, client gửi kèm token -> "nhận" luôn làm token chính thức (claim).
          nameRecordToWrite = { ...existingNameRecord, token };
        } else {
          const deviceId = body.deviceId;
          const metaDeviceMatches = existingMeta && existingMeta.deviceId && deviceId && existingMeta.deviceId === deviceId;
          if (!metaDeviceMatches) {
            return json({ error: 'Không xác thực được quyền sở hữu tài khoản "' + name + '" trên thiết bị này.' }, 403);
          }
        }

        // MỤC 22/31 (hardening): KHÔNG early-check expectedSavedAt ở đây (worker chính) để tránh 1 round-trip
        // KV thừa và — quan trọng hơn — vì early-check ở tầng HTTP vẫn có race window giữa đọc và ghi. Chốt
        // chặn THẬT SỰ nằm trong DO instance riêng của account (xem /internal/save-commit bên dưới).

        const data = body.data || {};
        try { validateSaveDataBasic(data); } catch (err) { return json({ error: 'Dữ liệu save bất thường, bị từ chối: ' + err.message }, 400); }

        // Chuẩn bị meta mới — giữ nguyên deviceId cũ nếu request không gửi kèm (không ghi đè bằng rỗng).
        const ip = getClientIp(request);
        const newDeviceId = body.deviceId || (existingMeta ? existingMeta.deviceId : null) || null;
        const metaToWrite = { ip, deviceId: newDeviceId, updatedAt: Date.now() };

        // MỤC 22/31 (hardening): CHỐT CHẶN THẬT SỰ chống lost-update — chuyển phần "đọc state hiện tại +
        // ghi state mới" cho 1 Durable Object instance RIÊNG của account này (getAccountLockStub), để 2
        // request /save gần như đồng thời của CÙNG account bị serialize tuần tự bởi chính Cloudflare, thay
        // vì chỉ optimistic-check ở tầng HTTP (vốn vẫn có race window giữa đọc và ghi). Account khác nhau đi
        // qua instance DO khác nhau nên không bị xếp hàng chung — không tạo cổ chai/self-DOS toàn server.
        // Nếu client gửi kèm expectedSavedAt, worker chính không đọc lại KV trước (tránh round-trip thừa) —
        // để DO tự so khớp NGAY TRONG instance tuần tự trước khi ghi (nếu lệch, trả lỗi SAVE_CONFLICT).
        const lockStub = getAccountLockStub(env, name);
        const commitResp = await lockStub.fetch(new Request('https://internal/internal/save-commit', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            metaToWrite,
            nameRecordToWrite, // undefined nếu không cần đổi (giữ nguyên bản KV hiện có) — JSON.stringify bỏ field undefined, DO hiểu là "không đổi"
          }),
        }));
        const commitJson = await commitResp.json();
        if (!commitResp.ok) return json(commitJson, commitResp.status);
        return json({ ok: true, savedAt: commitJson.savedAt });
      }
      ...
      if (path === '/leaderboard/top' && request.method === 'GET') {
        ...
        do {
          ...
          for (const k of page.keys) {
            ...
            if (!raw) continue;
            try {
              const d = looksLikeAesEnvelope(raw) ? await aesGcmDecryptFromJson(env, raw) : JSON.parse(raw);
      ...
      if (roomMatch) {
        ...
        if (roomMatch[2] === 'ws') {
          ...
          if (banReason) return json({ error: banReason }, 403);
          if (wsName && !(await isProtectedAdmin(env, wsName)) && await cachedKvGet(env, 'kicked:' + wsName, KV_CACHE_TTL_SHORT_MS)) {
      ...
      if (path === '/admin/is-temp' && request.method === 'GET') {
        ...
        const raw = await cachedKvGet(env, 'tempadmin:' + name, KV_CACHE_TTL_LONG_MS);
      ...
      if (path === '/trade/inventory' && request.method === 'GET') {
        ...
        const data = await readSaveRecord(env, name);
        if (!data) return json({ error: 'Không tìm thấy dữ liệu người chơi "' + name + '".' }, 404);
        return json({
          name,
          level: data.level || 1,
          gold: data.gold || 0,
      ...
      // ======================== ADMIN ========================
      // ---- POST /admin/verify  (Authorization: Bearer <ADMIN_KEY>) -> { ok, session? } ----
      // LUÔN xác thực bằng Admin Key GỐC (không phải isRealAdminRequest — session còn chưa tồn tại ở bước
      // này, đây chính là bước ĐỔI Admin Key LẤY session). Nếu server đã cấu hình ADMIN_PROTECT_SECRET,
      // cấp kèm 1 session token ký HMAC để dùng cho các lệnh admin tiếp theo (xem issueAdminSession).
      if (path === '/admin/verify' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'admin-verify', getClientIp(request), 5, 60))) return rateLimitedResponse(60);
        let body = {};
        try { body = await request.json(); } catch (err) { body = {}; }
        const ok = checkAdminKey(env, extractAdminKey(request, url, body));
        if (!ok) return json({ ok: false });
        const session = env.ADMIN_PROTECT_SECRET ? await issueAdminSession(env) : undefined;
      ...
      if (path === '/admin/command' && request.method === 'POST') {
        if (!(await checkRateLimit(env, 'admin-command', getClientIp(request), 30, 60))) return rateLimitedResponse(60);
        const body = await request.json();
        const { cmd, args = {} } = body;
        const isRealAdmin = isRealAdminRequest(env, request, url, body);
        let isTempAdmin = false;
        if (!isRealAdmin) {
          const actingName = normName(body.actingName);
          if (actingName) isTempAdmin = !!(await cachedKvGet(env, 'tempadmin:' + actingName, KV_CACHE_TTL_LONG_MS));
        }
      ...
      if (path === '/admin/players/all' && request.method === 'GET') {
        if (!isRealAdminRequest(env, request, url, null)) return json({ error: 'Sai admin key hoặc thiếu Admin Protect.' }, 403);
        let cursor;
        const players = [];
        do {
          const page = await env.GAME_DATA.list({ prefix: 'save:', cursor, limit: 1000 });
          for (const k of page.keys) {
            ...
            if (raw) {
              try {
                const sv = looksLikeAesEnvelope(raw) ? await aesGcmDecryptFromJson(env, raw) : JSON.parse(raw);
            ...
            const [bannedRaw, kickedRaw, isAdmin] = await Promise.all([
              cachedKvGet(env, 'banned:' + nname, KV_CACHE_TTL_SHORT_MS),
              cachedKvGet(env, 'kicked:' + nname, KV_CACHE_TTL_SHORT_MS),
      ...
      if (path === '/admin/event/set' && request.method === 'POST') {
        const body = await request.json();
        if (!isRealAdminRequest(env, request, url, body)) return json({ error: 'Sai admin key hoặc thiếu Admin Protect.' }, 403);
        const stub = getGlobalStub(env);
      ...
      if (path === '/chat/dm/send' && request.method === 'POST') {
        ...
        // Chống spam chat: tối đa 20 tin/10 giây theo IP — đủ cho chat bình thường, chặn được flood.
        if (!(await checkRateLimit(env, 'chat-dm-send', getClientIp(request), 20, 10))) return rateLimitedResponse(10);
      ...
      if (path === '/chat/group/create' && request.method === 'POST') {
        ...
        await cachedKvPut(env, 'group:' + groupId, JSON.stringify(group));
        for (const m of members) {
          const listRaw = await cachedKvGet(env, 'groups-of:' + m, KV_CACHE_TTL_LONG_MS);
          const list = listRaw ? JSON.parse(listRaw) : [];
          list.push(groupId);
          await cachedKvPut(env, 'groups-of:' + m, JSON.stringify(list));
        }
      ...
      if (path === '/chat/group/send' && request.method === 'POST') {
        const body = await request.json();
        // Chống spam chat nhóm: tối đa 20 tin/10 giây theo IP.
        if (!(await checkRateLimit(env, 'chat-group-send', getClientIp(request), 20, 10))) return rateLimitedResponse(10);
        const groupRaw = await cachedKvGet(env, 'group:' + body.groupId, KV_CACHE_TTL_LONG_MS);
      ...
      if (path === '/chat/group/history' && request.method === 'GET') {
        ...
        const groupRaw = await cachedKvGet(env, 'group:' + groupId, KV_CACHE_TTL_LONG_MS);
        if (!groupRaw) return json({ error: 'Nhóm không tồn tại.' }, 404);
        const group = JSON.parse(groupRaw);
        ...
        const raw = await env.GAME_DATA.get('group-chat:' + groupId); // lịch sử chat luôn đọc trực tiếp, không cache -> không bỏ lỡ tin nhắn mới nhất
        const history = raw ? JSON.parse(raw) : [];
        return json({ list: history.slice(-limit) });
      }
      ...
      if (path === '/chat/group/list' && request.method === 'GET') {
        ...
        const listRaw = await cachedKvGet(env, 'groups-of:' + name, KV_CACHE_TTL_LONG_MS);
        const groupIds = listRaw ? JSON.parse(listRaw) : [];
        const groups = [];
        for (const gid of groupIds) {
          const raw = await cachedKvGet(env, 'group:' + gid, KV_CACHE_TTL_LONG_MS);
      ...
      if (path === '/item/sell' && request.method === 'POST') {
        ...
        data[field] = Array.isArray(data[field]) ? data[field] : [];
        // FIX "không bán được Dark Blade thường (không phải Event)": trước đây indexOf so khớp CHÍNH XÁC
        // từng ký tự (phân biệt hoa/thường + khoảng trắng thừa), nên nếu người chơi gõ ô nhập tên vật phẩm
        // hơi khác 1 chút so với tên lưu trong túi đồ (VD "dark blade" thường thay vì "Dark Blade") thì bị
        // báo "Bạn không sở hữu..." dù rõ ràng đang có trong túi đồ. Nay so khớp không phân biệt hoa/thường
        // và bỏ khoảng trắng thừa hai đầu, đồng thời LUÔN dùng đúng tên gốc lưu trong data để xoá/định giá.
        const normalizedWanted = itemName.trim().toLowerCase();
        const idx = data[field].findIndex(it => String(it).trim().toLowerCase() === normalizedWanted);
        if (idx === -1) return json({ error: 'Bạn không sở hữu "' + itemName + '" trong ' + field + '.' }, 400);
        const actualItemName = data[field][idx];
        if (isSellForbidden(field, actualItemName)) return json({ error: '"' + actualItemName + '" không thể bán.' }, 400);
        const price = sellPriceFor(field, actualItemName);
        data[field].splice(idx, 1);
        data.gold = (data.gold || 0) + price;
        await writeSaveRecord(env, name, data, { forceFlush: true }); // item đã xoá khỏi túi đồ vĩnh viễn — ghi thẳng ngay
        return json({ ok: true, sold: actualItemName, field, price, gold: data.gold });
      }
